...
> 本轮13批40个GPU cases及后续观察器、T-098/T-100补包已复核；21个新单元功能/comparison/最终性能处置完成，总数各54。T-106为4/4：3项实测、1项用户接受的归因受限，后者不计收益或默认关闭免测。
> T-087设备解析、T-096精度修复已部署Pass回归；T-102为5/5安装态修复及性能完成，4改善、1回退。
...
- T-074 当前 188/158 均为 provisional；原33个冻结单元保留历史结论，本轮新增21个GPU冻结reference，
  新增21个NPU/comparison及21项性能处置，当前分别为54、54、54；其中1项为归因受限而非性能实测。产品改动合入及严格再认证单列。

## 当前门禁补强与历史复核

- [x] T-102共同候选五原方法、窄部署安装态五原方法、部署前后各六项设备边界通过；保留前后FX/IR/生成代码和旧失败。
- [x] T-102五个独立功能门禁、30个性能臂验签；1～4改善、5回退，3/4/5部分对齐。原先候选阶段清单仅为历史，现以[交付索引](results/current/T-102/README.md)为准。
- [x] 整理[部分交付说明](report/部分交付说明_20260915.md)，54/69闭环与未完成15项、历史再认证边界分别列示。
- [x] 17推理注册与15的序列化、replacement、extra_check及去重顺序静态核验，原ID留存并归并；71 ID / 69独立单元，已完成仍47。
- [x] 16/29具名GPU补测入口、AST窄适配、原helper数值/梯度与精确编号门禁、文本分片回传目录准备；不计设备通过。
- [ ] GPU执行16/29新补充入口并回传；不要求重跑17，见[一次性补测说明](report/remaining_work_20260915.md)。
- [x] 22号部署前边界回归：修正观察器 DeferredLine 错误后，NPU 6 基线及候选均22场景/23执行通过；首轮观察器失败及旧等锁超时原件保留。

- [x] 复核 2026-09-14 的 7 批 GPU 观察器包：28/28 原例，25 个本编号目标确认；历史复核另存不覆盖。
- [x] T-091 原生 NO_TESTS 后最小适配：真实 NPU `triton_experimental` 社区功能 1/1 通过，FX/IR/codegen 入库。
- [ ] 对已归因的 24 个 attention 单元逐例完成 NPU 功能、必要修复与性能门禁。
- [x] 重审 GPU 编号 16/17/29 的 reference 映射：16/17 推理退化、29 零 mask 消除，分别命中 14/5、15、30；17已确认去重，16/29新补测待执行。
- [x] T-091 独立精确目标 OFF/ON、六臂计时和正式 comparison：PERF_MIXED，不宣称稳定收益。
- [x] T-100 原社区完整方法（176次Tensor比较、160正例、16负例）、OFF/ON与六臂计时：PERF_IMPROVED；不改变默认开关。
- [x] T-098 默认模式真实失败留证；四进程诊断确认 HF32 模式与权重重参数化的舍入差异在 eager 对照中也存在。
- [ ] T-098 按 CUDA TF32-off 意图对齐 NPU HF32-off 后复验完整112组合（每组两次数值比较）；12600秒轮次已超时，完成84组合、最后169次Tensor比较通过，仅部分证据。此前35组合/70次比较的中断证据也保留，不据此外推全量通过。
- [x] T-102 pattern 1 历史安装态失败和隔离注册候选通过分开归档（20次Tensor比较、8次精确改写）；后续共同候选、边界、部署和性能已完成，旧失败保留。
- [x] T-102 pattern 2 注册候选 NPU 7 原方法通过：10次Tensor比较、4次精确改写、零skip；原安装态失败保留，见[注册候选报告](issues/REF-sfdp-pattern-2-native/注册候选验证报告.md)。
- [x] 用户允许确认空闲卡后并行功能测试；性能独占串行。允许候选/边界通过后备份窄改 experimental 安装文件并无候选复验，范围见 WORKFLOW。
- [x] 24个已归因attention安装态基线及适配复验：8原例通过、15训练合同未通过、22号真实数值失败；阶段数不计正式闭环。
- [x] 14/21/22/24数学展开代码断言适配和18号容器数值观察器复验完成；18为12个Tensor叶子比较，14训练仍未命中。
...
- [x] 2026-09-14新增历史日志已验旧run身份/逐文件SHA并重解析：24/24通过，非空日志缺口清零；不等于完整archive与更强数值认证通过。
- [x] T-078正确的FP16 value=1派生GPU单例补证通过；与NPU原修复证据对照后关闭邻接缺口，保留PARTIAL_ALIGNED和counter=0，不扩展性能分母。
...
完成“核验并列出缺口”不等于全部证据补齐；历史日志本轮已重解析24/24通过，但全archive、5项oracle边界及NPU/性能运行溯源仍待补。
...
- [x] 将 T-074 的 48 个 `no-test-found` 和 29 个 indirect coverage hint 全部纳入
  T-078～T-113 逐批审核；可形成真实合同者进入 manifest，其余登记明确延期理由；
- [x] 优先从 community tests 反向识别 optimization contracts；
- [x] 判断多个 registration 是否属于一个 contract；
- [x] 判断一个 registration 是否展开多个必须分别验证的 variants；
- [x] 无法确认独立设备合同时保留 `deferred` 和重开条件，不猜测、不制造 GPU/NPU 路径；
- [ ] 完成 lowering/template 独立 inventory 并与 FX 合同去重后，再决定是否生成 T-074 v2；
...
- [x] T-091/T-096已有原生GPU回传；T-091目标pre-grad归因于2026-09-14补齐并完成NPU与性能；
...
- [x] 接收并复核T-098/T-100原run重新压缩补包：不是新GPU运行；T-100已闭环，T-098完整精度模式复验仍待完成（短限额轮次中断留证）；
- [x] T-102～T-107和T-112均已回传完整GPU包，T-102全部11片已校验；
- [x] 为T-091/T-102～T-107接入只读精确handler/replacement边界图观察，不修改原方法体、device或断言；
- [x] GPU已用更新入口补精确归因并逐编号核验：25个确认、16/17/29映射不符；旧原生PASS保留；3项新目标合同仍待单独补证；
...
4. [x] 接收并复核 GPU 文本 artifacts；no-test-found 和 indirect 映射随后已在
   T-078～T-113 全部完成逐批审核与延期处置；
