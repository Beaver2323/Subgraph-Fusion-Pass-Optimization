...
> 本轮13批40个GPU cases及后续观察器、T-098/T-100补包已复核；14个新单元功能/comparison/性能处置完成，总数各47。
> T-087设备解析、T-096精度修复已部署Pass回归；T-102 pattern 1只有隔离候选通过，不能并入安装态完成数。
...
- T-074 当前 188/158 均为 provisional；原33个冻结单元保留历史结论，本轮新增14个GPU冻结reference，
  新增14个NPU/comparison及14项性能处置，当前分别为47、47、47；产品改动合入及严格再认证单列。
...
- [ ] 22号独立设备边界回归：9月15日09:05确认等待项目锁超时，未执行设备；需重新调度，候选/原安装态各独立进程，不自动部署。
...
- [x] T-102 pattern 1 安装态失败和隔离注册候选通过分开归档（20次Tensor比较、8次精确改写）；尚未授权部署，不计安装态修复。
