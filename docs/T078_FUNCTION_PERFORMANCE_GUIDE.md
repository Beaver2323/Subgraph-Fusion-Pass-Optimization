...
NPU 修复保持窄范围：仅 NPU、FP32/BF16、非 Tensor scalar；FP16 和其他 dtype 不放开。

源码覆盖审查新增 FP16/BF16 两个 dtype-only variants，并在 GPU 上复用相同
`64x64/value=2/bitwise/counter/codegen` 合同。NPU 同输入三臂证明 BF16 合法、FP16 存在 pass 外的
compiled/eager 精度阻断；专项命令、代码和证据清单见
