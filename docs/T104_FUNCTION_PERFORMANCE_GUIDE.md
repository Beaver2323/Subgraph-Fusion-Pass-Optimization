...
意图：将显式转置后的链改为SDPA。GPU执行 `test/inductor/test_fused_attention.py::SDPAPatternRewriterGpuTests.test_sdpa_rewriter_11_gpu`；必须把通用`fuse_attention`计数与精确`_sfdp_pattern_11` counter/FX对应起来。NPU目前仅为待验证：必须在`triton_experimental`下证明同编号命中、数值正确且无fallback，不能借用CUDA或其他NPU backend结论。
...
意图：训练态保留dropout概率。GPU执行 `test/inductor/test_fused_attention.py::SDPAPatternRewriterGpuTests.test_sdpa_rewriter_12_gpu`；必须把通用`fuse_attention`计数与精确`_sfdp_pattern_12` counter/FX对应起来。NPU目前仅为待验证：必须在`triton_experimental`下证明同编号命中、数值正确且无fallback，不能借用CUDA或其他NPU backend结论。
...
意图：用unsqueeze/squeeze映射到四维SDPA，且检查permute维。GPU执行 `test/inductor/test_fused_attention.py::SDPAPatternRewriterGpuTests.test_sdpa_rewriter_13_gpu`；必须把通用`fuse_attention`计数与精确`_sfdp_pattern_13` counter/FX对应起来。NPU目前仅为待验证：必须在`triton_experimental`下证明同编号命中、数值正确且无fallback，不能借用CUDA或其他NPU backend结论。
...
意图：转置QKV并传递同dtype/bool mask。GPU执行 `test/inductor/test_fused_attention.py::SDPAPatternRewriterGpuTests.test_sdpa_rewriter_14_gpu`；必须把通用`fuse_attention`计数与精确`_sfdp_pattern_14` counter/FX对应起来。NPU目前仅为待验证：必须在`triton_experimental`下证明同编号命中、数值正确且无fallback，不能借用CUDA或其他NPU backend结论。
...
意图：把0/1 mask规范为SDPA布尔mask。GPU执行 `test/inductor/test_fused_attention.py::SDPAPatternRewriterGpuTests.test_sdpa_rewriter_15_gpu`；必须把通用`fuse_attention`计数与精确`_sfdp_pattern_15` counter/FX对应起来。NPU目前仅为待验证：必须在`triton_experimental`下证明同编号命中、数值正确且无fallback，不能借用CUDA或其他NPU backend结论。
