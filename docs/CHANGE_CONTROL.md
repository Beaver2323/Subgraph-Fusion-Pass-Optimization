...
- 2026-09-07 22:22 CST：用户授权两个并行线程，分别核验 T-076/T-077 与准备
  ...
  主线程集成统一门禁与状态导航，补齐嵌套 source overlay 的语法检查。历史证据只追加审计，
  不补造运行元数据、不扩大冻结分母；本项不授权改动或安装产品仓库。状态：approved。
  22:47 集成验证：120 项标准库回归、8 批 reference 静态入口、计划/矩阵/backlog 与递归
  语法检查通过；T-081～T-083 共 7 单元/11 cases/24 variants 为待 GPU 状态，冻结仍为 21。
  历史核验已完成现存证据审阅，严格门禁仍为 41 pending / 3 exempt；详见
  `report/t076_t077_history_reaudit_20260907.md`。本轮未运行设备、未修改产品源码。状态：verified-tooling-only。
...
- 受限回传：GPU 机器禁用 Git/二进制上传。新增 `scripts/export_reference_text.py`，将任意 reference
  run 导出为单个 UTF-8 JSON，包含完整环境与 summary、逐 case 执行/FX signature、关键文件大小和
...
### E-223：GPU 功能默认共享与快速等卡（2026-09-06）

- 登记时间：2026-09-06 05:20 CST（UTC+08:00）。按用户要求，统一 GPU 功能入口改为默认共享；
  DEFAULT 计算模式下不因已有计算进程或高利用率拒绝启动。`--exclusive` 保留启动时无计算进程的策略。
- 新增 `--wait-gpu`、`--wait-timeout`、`--poll-interval` 与 `--min-free-memory-mib`；
  默认每 1 秒检查指定卡，显存门槛 1024 MiB，可设为 0。排队可取消，有超时和查询失败退出码。
- 共享/独占协作锁覆盖入口运行及导出周期，只约束使用同一锁目录的 tracker，不宣称整机独占或 FIFO。
  不终止其他 GPU 进程、不修改计算模式/驱动；共享功能耗时不计正式性能收益。
- 自动 run 目录增加随机后缀防止同秒并发冲突；显式 run-id 仍禁止覆盖。运行策略及显存检查值进入
  环境指纹/文本 handoff。保留固定 latest；启动前等待/拒绝不会发布新结果，并发时以最后发布轮次为准。
- 通用说明与 T-076～T-080 操作说明同步为中文带时间戳文档，补齐控制节点 `results/incoming/` 接收入口。
  审计规则升级为 `2026-09-06.2`；NPU `triton_experimental`、性能合同和历史结果不变。
- 零设备统一门禁通过：66 项单元/集成测试、53 个 reference cases 的静态计划有效；新测试使用模拟
  GPU 查询，不占用真实 GPU/NPU。历史再认证仍有 41 项 pending、3 项关闭免测，未将其转为 passed。

### E-224：补齐远端 handoff 接收目录（2026-09-06）

- 登记时间：2026-09-06 06:00 CST（UTC+08:00）。纠正只有文档路径、没有受版本控制目录的问题：
  `results/incoming/` 新增中文接收说明，T-076～T-080 使用 `.gitkeep` 保留，clone/pull 后直接可见。
- 收紧 Git 例外为说明和占位文件；实际 handoff JSON、日志与嵌套原始证据继续默认忽略，
  不自动视为正式结果，也不要求为了 Agent 读取而上传。
- 新增目录存在性与忽略边界回归，统一零设备门禁 68 项测试通过；未运行 GPU/NPU。
  追加审计仍保留 41 项历史 pending、3 项关闭免测，不修改历史验收结论。

### E-225：文本 handoff 1.1 原文回传（2026-09-06）

- 登记时间：2026-09-06 06:43 CST（UTC+08:00）。解决 1.0 紧凑包只能看到 FX
  count/signature/hash、不能读取 GPU 实际 FX/日志/生成代码正文的问题。
- `export_reference_text.py --include-raw-text` 将 inventory 登记的 UTF-8 FX、日志、JSON、生成代码和
  常见 Triton/LLVM/PTX IR 原文嵌入单个 JSON；二进制只保留路径、大小、哈希和缺项原因，64 MiB
  超限直接失败，不截断证据。GPU 统一一键入口默认生成格式 1.1，手工省略该开关仍兼容格式 1.0。
- 新增只读导入器：先验证整包 SHA256、逐文件大小/哈希、summary 原文和 case inventory 绑定，再在
  tracker 仓库外的新目录恢复文本；拒绝路径穿越、`.git`、控制字符、重复路径、软链接来源与覆盖，
  永不导入或执行回传代码。`--validate-only` 可只校验、不落盘。
...
### E-228：文本 handoff 1.2 压缩传输（2026-09-07）

- 登记时间：2026-09-07 06:02 CST（UTC+08:00）。1.1 将 FX、日志和生成代码逐字放入 JSON，证据
  完整但网页手工复制过长；新增 1.2，将每个 UTF-8 原文件用 zlib 压缩后 Base64 编码。
- 原始文件路径、字节数、SHA256、inventory 绑定与整包 `payload_sha256` 保持不变；控制节点在
  有长度上限的方式下解压、校验并逐字节恢复。导入器不执行回传代码，并继续兼容旧 1.1。
- GPU 一键入口默认使用 `--include-raw-text --compress-raw-text --compact`；已有历史 run 可以直接
...
### E-230：网页分片上限二次收束（2026-09-07）

- 登记时间：2026-09-07 07:17 CST（UTC+08:00）。T-079 首版实际生成 1401 字节 manifest、三个约
  263 KiB 分片和一个约 184 KiB 分片，仍被当前 GitHub 网页编辑通道拒绝，证明 192 KiB 原始
  分片在该通道上不是可用上限。
- 默认原始分片由 192 KiB 降到 48 KiB，Base64 行由 4096 字符降到 1024 字符；成品通常约
