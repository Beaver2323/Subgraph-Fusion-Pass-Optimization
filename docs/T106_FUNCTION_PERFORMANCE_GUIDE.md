...
> 状态：本批4/4功能/comparison及最终处置完成。3项性能实测（22回退、23改善、24回退），1项用户接受的归因受限（21）；默认关闭免测0项。不是4项性能实测。

[21号功能与归因限制讲解](../results/current/T-106/pattern-21_讲解.md)：用户接受“功能通过、单pattern性能无法独立归因”结项；不计收益，不计默认关闭免测。22号正常性能验收已完成，结论不变。
