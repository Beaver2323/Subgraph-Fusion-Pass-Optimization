...
> T-087～T-113保留38个跟踪ID，T-112及pattern17去重后为36个独立单元；其中21个已闭环，15个尚未闭环。
> 13批40个GPU cases及观察器/补包已复核；21个新单元完成NPU功能、comparison和性能处置；
> T-081～T-113已无草案批次，所有未选候选均保留明确的合并/延期原因。

## 1. 总结

2026-09-15 20:20更新：**正式闭环54/69，T-102为5/5完成，可部分交付**。
共同候选及安装态各五个原社区方法均通过，部署前后各六项设备边界通过；五项独立OFF/ON及30个新进程性能臂均已验签。
1～4为PERF_IMPROVED，5为PERF_REGRESSED：host p50/p99时延增加40.27%/29.91%，Event增加53.11%/42.08%。
本批均为half训练图的前向微基准，不含backward或模型端到端。3/4原社区随机数值oracle缺失，5为数学展开，三项明确PARTIAL_ALIGNED。
最小训练注册修复已窄部署Pass，本地产品提交`3a0b817`；未推送产品仓、发起PR或运行社区CI，不宣称新wheel认证。
T-076/T-077严格历史再认证仍41项pending、3项exempt；其他批次保留各自安装源码指纹，未因本轮部署自动重验。
入口：[T-102交付与逐pattern讲解](../results/current/T-102/README.md)、[部分交付说明](../report/部分交付说明_20260915.md)。下方保留阶段历史。

2026-09-15 17:21更新：**正式闭环49/69，T-106为4/4已处置**。
用户明确接受21号“功能通过、单pattern性能无法独立归因”结项；状态
`PERF_NOT_INDEPENDENTLY_ATTRIBUTABLE`。T-106性能拆分为3项实测（1改善、2回退）、
1项归因受限、0项默认关闭免测；21号不计性能收益、不产生新计时数据。
原OFF退出码1和22号接替改图证据保留，不能将本次确认说成修复OFF或性能通过。
22号修复/性能回退结论均不变。[21号讲解与确认](../results/current/T-106/pattern-21_讲解.md)。

2026-09-15 11:56更新：**正式闭环48/69，T-106为3/4**。22号修复后同NPU5完整原例、
独立OFF/ON和六臂性能已通过验签；性能PERF_REGRESSED，host p50/p99时延增加22.94%/25.86%，
Event增加29.19%/27.99%。保留正确性修复，不改默认配置；数学展开与CUDA路径差异标PARTIAL_ALIGNED。
21号安装态原例通过，但新OFF仍被22接替，未计时、未闭环，也不是产品默认关闭免测。
23/24的历史性能结果保留原源码指纹，本轮新增邻接功能不等于重新认证其性能。
产品修复分支本地提交478bcb5，未推送产品仓/合入/运行社区CI。
完整入口：[22号学习与性能报告](../results/current/T-106/pattern-22_讲解.md)、
[21号阻断证据](../issues/REF-sfdp-pattern-21-native/性能归因阻断.md)。以下按时间保留历史进度，不覆盖旧失败。

2026-09-15 11:02更新：用户已允许空闲卡并行功能及条件窄部署。pattern22的部署前基线、
候选均完成22场景/23次边界设备执行并通过；修正过一次DeferredLine观察器错误，失败原件保留。
仅两个select-load方法已备份后部署到Pass；NPU6安装态边界22场景/23执行、NPU7原例及21/23/24邻接21次Tensor/9次改写均无候选新进程通过。性能待验收，正式闭环仍47/69。
pattern2注册候选完整原方法通过（10次Tensor、4次精确改写），但未部署。
全流程完成仍47/69，不把已部署或候选通过计成新增闭环。
最新入口：[部署与边界回归](../issues/REF-sfdp-pattern-22-native/部署与边界回归报告.md)。
