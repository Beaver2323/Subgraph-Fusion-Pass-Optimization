...
2026-09-15 21:49补充：面向人的进度表使用直观中文，禁止把内部状态码直接作为展示用语。
“编译阶段”和“完成情况”分列，尤其不能把freezing误读为任务未完成；覆盖完成只指已登记测试范围。
GPU对照有效、NPU功能通过、社区一致性和性能结论分别解释，不能互相替代。中文映射由生成器维护并测试，
CSV/JSON保留稳定机器代码和原始证据，不为翻译改写历史结论。说明入口：[矩阵字段中文说明](docs/矩阵字段说明.md)。
...
2026-09-15 用户明确授权本轮剩余修复：候选与边界回归通过后，先备份、仅窄改 Pass 安装包对应的
`triton_experimental` 文件，再以不加载候选的新进程验证。不得改冻结的上游 PyTorch，
不得强开明确禁用路径；测试/编译进程全部结束后才能修改公共安装文件。
另允许确认空闲 NPU 后分卡并行功能测试；性能仍独占串行。功能任务使用性能锁的共享模式，
同卡使用设备独占锁，性能使用全局独占锁。协作锁不替代 `npu-smi` 空闲检查，不能终止其他用户进程。

公共codegen修复后的性能门禁必须重新绑定已验证部署的SHA、同卡完整原社区合同、独立OFF/ON和六臂。
