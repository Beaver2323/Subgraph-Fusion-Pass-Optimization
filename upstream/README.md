...
> 状态：T-076～T-078 共 14 units 已形成正式 NPU/comparison；T-079 的 4 units/4 cases/14 variants 已冻结 GPU reference、等待 NPU；T-080 的 3 units/13 cases/13 variants 等待 GPU reference。
...
- T-076～T-079 的 18 个单元为 `yes-frozen`；其中 T-076～T-078 的 14 个已形成正式
  NPU/comparison，T-079 的 4 个等待 NPU；T-080 的 3 个仍为 `pending-reference`；
...
- T-079 的 4 个单元已由 4/4 direct GPU cases 和可恢复 FX 正文冻结；T-080 的 3 个单元仍为
  `pending-reference`。T-080 又纠正 const-scatter 与 constructor mover 两个 `no-test-found`，并登记
  社区 CrossEntropy/softmax 性能方法供功能门禁后复用。
