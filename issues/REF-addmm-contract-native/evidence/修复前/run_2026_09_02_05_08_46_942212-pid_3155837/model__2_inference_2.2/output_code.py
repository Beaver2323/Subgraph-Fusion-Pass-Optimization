...
# kernel path: /home/z50063656/tmp/t076-npu-results/REF-addmm-contract-native/adapter-20260902T051000+0800/cache/sb/csbxigid67k2r6enx6nsrtayaspnebk6sbaumnr3bujnyxkecupc.py
...
#   %mm : Tensor "f32[1, 16][16, 1]npu:0" = PlaceHolder[target=mm]
#   %mm_1 : Tensor "f32[1, 16][16, 1]npu:0" = PlaceHolder[target=mm_1]
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 256}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'in_ptr2': '*fp32', 'out_ptr0': '*fp32', 'out_ptr1': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3, 4, 5), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [16, 16, {'divisors': [1, 16]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 16, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 16, 'divisor': 16, 'seed': 2}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_add_0', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 3, 'num_reduction': 0, 'npu_num_x_nodes': 2, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_add_0(in_ptr0, in_ptr1, in_ptr2, out_ptr0, out_ptr1, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    ...
    for i in range(group_size):
        ...
        xmask = x0mask & x1mask
        tmp0 = tl.load(in_ptr0 + (x0 + 16*x1), x0mask & x1mask)
        tmp1 = tl.load(in_ptr1 + (x0), x0mask, eviction_policy='evict_last')
        tmp3 = tl.load(in_ptr2 + (x0), x0mask, eviction_policy='evict_last')
        tmp2 = tmp0 + tmp1
        tmp4 = tmp3 + tmp0
        tl.store(out_ptr0 + (x0 + 16*x1), tmp2, x0mask & x1mask)
        tl.store(out_ptr1 + (x0 + 16*x1), tmp4, x0mask & x1mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((1, 16), (16, 1), torch.float32)
            ...
            buf2 = empty_strided_npu((1, 16), (16, 1), torch.float32)
            ...
            buf1 = empty_strided_npu((16, 16), (16, 1), torch.float32)
            buf3 = empty_strided_npu((16, 16), (16, 1), torch.float32)
            # Topologically Sorted Source Nodes: [add, add_1], Original ATen: [aten.add]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_0.run(arg2_1, buf0, buf2, buf1, buf3, 256, stream=raw_stream0)
            del arg2_1
            del buf0
            del buf2
        return (buf1, buf3, )
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 16), (16, 1), device='npu:0', dtype=torch.float32)
