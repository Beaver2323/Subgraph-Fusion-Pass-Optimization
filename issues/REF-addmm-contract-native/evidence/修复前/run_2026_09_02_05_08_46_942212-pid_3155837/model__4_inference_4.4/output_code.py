...
# kernel path: /home/z50063656/tmp/t076-npu-results/REF-addmm-contract-native/adapter-20260902T051000+0800/cache/pq/cpqmqe35xfktcqc4crn2gvsoajxdvedhwpyli5ohxnzxlotkkuuf.py
# Topologically Sorted Source Nodes: [add], Original ATen: [aten.add]
# Source node to ATen node mapping:
#   add => add
# Graph fragment:
#   %mm : Tensor "f32[16, 16][16, 1]npu:0" = PlaceHolder[target=mm]
#   %add : Tensor "f32[16, 16][16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (4, %mm), kwargs = {})
#   return %add
...
@triton.jit
def triton_unk_fused_add_0(in_out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    ...
    for i in range(group_size):
        ...
        tmp1 = tl.full([1], 4.0, tl.float32)
        tmp2 = tmp1 + tmp0
        tl.store(in_out_ptr0 + (x0), tmp2, x0mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t076-npu-results/REF-addmm-contract-native/adapter-20260902T051000+0800/cache/oo/cooxayjyrifj73qzrdi7koxzu4xadlh2oa7i6tsigj4mquo3gkch.py
# Topologically Sorted Source Nodes: [add_1], Original ATen: [aten.add]
# Source node to ATen node mapping:
#   add_1 => add_1
# Graph fragment:
#   %mm_1 : Tensor "f32[16, 16][16, 1]npu:0" = PlaceHolder[target=mm_1]
#   %add_1 : Tensor "f32[16, 16][16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mm_1, 4), kwargs = {})
#   return %add_1
triton_unk_fused_add_1 = async_compile.triton('triton_unk_fused_add_1', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [256, {'divisors': [1]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 256, 'divisor': 1, 'seed': 1}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_add_1', 'mutated_arg_names': ['in_out_ptr0'], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 1, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_add_1(in_out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 256
    x_g_tile0 : tl.constexpr = (256) if (256) < (XBLOCK) else (XBLOCK)
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused_add_0.run(buf1, 256, stream=raw_stream0)
            ...
            # Topologically Sorted Source Nodes: [add_1], Original ATen: [aten.add]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_1.run(buf3, 256, stream=raw_stream0)
        return (buf1, buf3, )
