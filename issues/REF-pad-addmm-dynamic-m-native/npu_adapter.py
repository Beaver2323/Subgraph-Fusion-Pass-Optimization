#!/usr/bin/env python3
"""dynamic-M pad-addmm community case 的 NPU 产品基线适配器。"""
...
def load_upstream(root: Path):
    ...
    spec = importlib.util.spec_from_file_location("t076_pad_addmm_dynamic", source)
...
def main() -> int:
    ...
    class Case(unittest.TestCase):
        ...
        def test_product_baseline(self):
            ...
            m, k, n = 128, 33, 40
            inputs = (
                torch.randn(m, n, device="npu", dtype=torch.float32),
                torch.randn(m, k, device="npu", dtype=torch.float32),
                torch.randn(k, n, device="npu", dtype=torch.float32),
            )
            aligned_k = upstream.get_padded_length(k, upstream.get_alignment_size(inputs[1])) + k
            ...
            observation.update({"input_shapes": [list(x.shape) for x in inputs], "input_strides": [list(x.stride()) for x in inputs], "dynamic_dimensions": ["bias.dim0", "mat1.dim0"], "reference_positive_marker": marker, "reference_positive_marker_present": False, "disable_pad_mm": ncfg.disable_pad_mm, "shape_padding_after_backend_load": upstream.inductor_config.shape_padding, "correctness": "passed", "generated_code": str(code_path)})

    result = unittest.TextTestRunner(verbosity=2).run(unittest.TestSuite([Case("test_product_baseline")]))
    summary = {"source_test": "test/inductor/test_pad_mm.py::PadMMTest.test_pad_addmm_dyn_m", "adapter_deviation": ["GPU_TYPE=npu", "backend=triton_experimental", "bypass GPU-only harness", "expand TRITON to TRITON,ATEN", "map padded-K positive to product-gate negative"], "product_gate_bypassed": False, "tests_run": result.testsRun, "failures": len(result.failures), "errors": len(result.errors), "skipped": len(result.skipped), "successful": result.wasSuccessful(), "torch_version": torch.__version__, "torch_npu_version": torch_npu.__version__, "device": torch.npu.get_device_name(0), "observation": observation, "selected_environment": {key: os.environ.get(key) for key in ("ASCEND_RT_VISIBLE_DEVICES", "SET_NPU_DEVICE", "TORCH_COMPILE_DEBUG", "TORCH_COMPILE_DEBUG_DIR", "TORCH_TRACE", "TORCHINDUCTOR_CACHE_DIR")}}
