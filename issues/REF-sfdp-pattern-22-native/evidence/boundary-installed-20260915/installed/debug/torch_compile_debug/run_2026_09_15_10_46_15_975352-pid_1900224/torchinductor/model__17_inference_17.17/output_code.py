...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-5k498lwi/installed/inductor-cache/tmp1dsvtlcf/a4/ca4cgpnfa7lhnknvoetxqhadq7c7qs3n3vjtedj23litbvs4o7t3.py
...
#   unsqueeze => unsqueeze
# Graph fragment:
#   %arg0_1 : Tensor "bf16[1, 7, 8][56, 8, 1]npu:0" = PlaceHolder[target=arg0_1]
#   %select : Tensor "bf16[1, 7][56, 8]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 7), kwargs = {})
#   %unsqueeze : Tensor "bf16[1, 7, 1][56, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
#   %add : Tensor "bf16[3, 7, 5][35, 5, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%unsqueeze, %arg1_1), kwargs = {})
#   return %buf0
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 105}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*bf16', 'out_ptr0': '*bf16', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [5, 7, 3, {'divisors': [1, 5, 35]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 5, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 7, 'divisor': 5, 'seed': 2}, {'name': 'x2', 'length': 3, 'divisor': 35, 'seed': 2}]},
...
@triton.jit
def triton_unk_fused_add_select_unsqueeze_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    ...
    x0numel : tl.constexpr = 5
    x1numel : tl.constexpr = 7
    ...
    for i in range(group_size):
        ...
        _es_lane0 = tl.arange(0, 8)[None, None, :]
        _es_full0 = tl.load(in_ptr0 + (7 + 8 * x1) + _es_lane0, x1mask & ((7 + 8 * x1 + _es_lane0 >= 0) & (7 + 8 * x1 + _es_lane0 < 56)), eviction_policy='evict_last').to(tl.float32)
        tmp0 = extract_slice(_es_full0, [0, 0, 0], [_es_full0.shape[0], _es_full0.shape[1], 1], [1, 1, 1])
        tl.store(out_ptr0 + (x0 + 5*x1 + 35*x2), tmp0, x0mask & x1mask & x2mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/attention-slice-boundary-5k498lwi/installed/inductor-cache/tmp1dsvtlcf/yb/cyb2ugtdopnvob6cu6u7xa2hkjxkvquhs42qllz7mu7mjkzrsf6d.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
#   unsqueeze => unsqueeze
# Graph fragment:
#   %buf0 : Tensor "bf16[3, 7, 5][35, 5, 1]npu:0" = PlaceHolder[target=buf0]
#   %arg1_1 : Tensor "bf16[3, 7, 5][35, 5, 1]npu:0" = PlaceHolder[target=arg1_1]
#   %select : Tensor "bf16[1, 7][56, 8]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 7), kwargs = {})
#   %unsqueeze : Tensor "bf16[1, 7, 1][56, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
#   %add : Tensor "bf16[3, 7, 5][35, 5, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%unsqueeze, %arg1_1), kwargs = {})
#   return %add
triton_unk_fused_add_select_unsqueeze_1 = async_compile.triton('triton_unk_fused_add_select_unsqueeze_1', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 105}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*bf16', 'in_ptr0': '*bf16', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [105, {'divisors': [1]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 105, 'divisor': 1, 'seed': 1}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_add_select_unsqueeze_1', 'mutated_arg_names': ['in_out_ptr0'], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 2, 'num_reduction': 0, 'npu_num_x_nodes': 1, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_add_select_unsqueeze_1(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 105
    x_g_tile0 : tl.constexpr = (105) if (105) < (XBLOCK) else (XBLOCK)
    x0numel : tl.constexpr = 105
    ...
    for i in range(group_size):
        ...
        tmp1 = tl.load(in_ptr0 + (x0), x0mask).to(tl.float32)
        tmp2 = tmp0 + tmp1
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((3, 7, 5), (35, 5, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_select_unsqueeze_0.run(arg0_1, buf0, 105, stream=raw_stream0)
            del arg0_1
            arg1_1 = copy_if_misaligned(arg1_1)
            buf1 = buf0; del buf0  # reuse
            # Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_select_unsqueeze_1.run(buf1, arg1_1, 105, stream=raw_stream0)
            del arg1_1
        return (buf1, )
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 7, 8), (56, 8, 1), device='npu:0', dtype=torch.bfloat16)
    arg1_1 = rand_strided((3, 7, 5), (35, 5, 1), device='npu:0', dtype=torch.bfloat16)
