...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-5k498lwi/installed/inductor-cache/tmppduuulvv/ei/ceif3ozxbaomjcqp43pfr7yxyuvwjfio4eiih3yc6pmadxk5mpfa.py
...
#   unsqueeze => unsqueeze
# Graph fragment:
#   %arg0_1 : Tensor "bf16[1, 7, 2][14, 2, 1]npu:0" = PlaceHolder[target=arg0_1]
#   %select : Tensor "bf16[1, 7][14, 2]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 1), kwargs = {})
#   %unsqueeze : Tensor "bf16[1, 7, 1][14, 2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
...
def triton_unk_fused_add_select_unsqueeze_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + (1 + 2 * x1) + _es_lane0, x1mask & ((1 + 2 * x1 + _es_lane0 >= 0) & (1 + 2 * x1 + _es_lane0 < 14)), eviction_policy='evict_last').to(tl.float32)
...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-5k498lwi/installed/inductor-cache/tmppduuulvv/yb/cyb2ugtdopnvob6cu6u7xa2hkjxkvquhs42qllz7mu7mjkzrsf6d.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 7, 2), (14, 2, 1), device='npu:0', dtype=torch.bfloat16)
