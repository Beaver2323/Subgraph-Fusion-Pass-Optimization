...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmprvgdknih/gj/cgjsbc3wid3bxkrnkkjujrpme7rwoka5p5baph4f3d5p7vfs4py5.py
...
#   %select : Tensor "bf16[1, 7][14, 2]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 0), kwargs = {})
...
def triton_unk_fused_add_select_unsqueeze_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + 2 * x1 + _es_lane0, x1mask & ((2 * x1 + _es_lane0 >= 0) & (2 * x1 + _es_lane0 < 14)), eviction_policy='evict_last').to(tl.float32)
...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmprvgdknih/yb/cyb2ugtdopnvob6cu6u7xa2hkjxkvquhs42qllz7mu7mjkzrsf6d.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
