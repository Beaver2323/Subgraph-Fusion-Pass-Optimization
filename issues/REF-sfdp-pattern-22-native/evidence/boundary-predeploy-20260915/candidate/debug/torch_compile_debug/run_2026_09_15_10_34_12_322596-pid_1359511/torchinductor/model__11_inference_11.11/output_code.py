...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmptkhro1nn/2f/c2fzgyhqavevkqqcjotdoe744syovj4ezu4aj2k4k2ow32dgfp6w.py
...
#   unsqueeze => unsqueeze
# Graph fragment:
#   %arg0_1 : Tensor "f16[1, 7, 8][56, 8, 1]npu:0" = PlaceHolder[target=arg0_1]
#   %select : Tensor "f16[1, 7][56, 8]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 7), kwargs = {})
#   %unsqueeze : Tensor "f16[1, 7, 1][56, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
#   %add : Tensor "f16[3, 7, 5][35, 5, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%unsqueeze, %arg1_1), kwargs = {})
#   return %buf0
...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmptkhro1nn/6t/c6taaq32hist6mij5lbolirtlkl3jercufvw5hu3cadvgsbrigey.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
#   unsqueeze => unsqueeze
# Graph fragment:
#   %buf0 : Tensor "f16[3, 7, 5][35, 5, 1]npu:0" = PlaceHolder[target=buf0]
#   %arg1_1 : Tensor "f16[3, 7, 5][35, 5, 1]npu:0" = PlaceHolder[target=arg1_1]
#   %select : Tensor "f16[1, 7][56, 8]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 7), kwargs = {})
#   %unsqueeze : Tensor "f16[1, 7, 1][56, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
#   %add : Tensor "f16[3, 7, 5][35, 5, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%unsqueeze, %arg1_1), kwargs = {})
#   return %add
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((3, 7, 5), (35, 5, 1), torch.float16)
            # Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 7, 8), (56, 8, 1), device='npu:0', dtype=torch.float16)
    arg1_1 = rand_strided((3, 7, 5), (35, 5, 1), device='npu:0', dtype=torch.float16)
