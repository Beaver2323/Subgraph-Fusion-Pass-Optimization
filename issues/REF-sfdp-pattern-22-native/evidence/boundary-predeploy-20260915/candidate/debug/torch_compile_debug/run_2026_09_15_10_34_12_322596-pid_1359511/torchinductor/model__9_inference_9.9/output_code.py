...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmpss2ihr96/hm/chmcpco4nizw4azruydndooinehsn6vm52uwf7nr53cfgyxl6l3q.py
...
#   unsqueeze => unsqueeze
# Graph fragment:
#   %arg0_1 : Tensor "f16[1, 7, 4][28, 4, 1]npu:0" = PlaceHolder[target=arg0_1]
#   %select : Tensor "f16[1, 7][28, 4]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 3), kwargs = {})
#   %unsqueeze : Tensor "f16[1, 7, 1][28, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmpss2ihr96/6t/c6taaq32hist6mij5lbolirtlkl3jercufvw5hu3cadvgsbrigey.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 7, 4), (28, 4, 1), device='npu:0', dtype=torch.float16)
