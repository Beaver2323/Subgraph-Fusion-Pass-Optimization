...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmp5vm44x25/4e/c4eum53axsisy6cxqvvxlabcjn46dkiaqx5ygj4u7rajbuh7j6pe.py
...
#   unsqueeze => unsqueeze
# Graph fragment:
#   %arg0_1 : Tensor "f32[1, 7, 4][28, 4, 1]npu:0" = PlaceHolder[target=arg0_1]
#   %select : Tensor "f32[1, 7][28, 4]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 3), kwargs = {})
#   %unsqueeze : Tensor "f32[1, 7, 1][28, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
...
def triton_unk_fused_add_select_unsqueeze_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + (3 + 4 * x1) + _es_lane0, x1mask & ((3 + 4 * x1 + _es_lane0 >= 0) & (3 + 4 * x1 + _es_lane0 < 28)), eviction_policy='evict_last')
...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmp5vm44x25/ok/coke2a2cmbgoc44epklqqct5q6vsanpq5ntjpm6uwnq5wlkydqb6.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 7, 4), (28, 4, 1), device='npu:0', dtype=torch.float32)
