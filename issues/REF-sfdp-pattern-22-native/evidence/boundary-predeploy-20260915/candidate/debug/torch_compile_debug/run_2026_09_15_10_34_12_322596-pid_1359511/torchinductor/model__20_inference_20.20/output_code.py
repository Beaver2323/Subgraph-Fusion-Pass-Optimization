...
# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmpsrdaphv8/5f/c5fvyd3lngqlwyuu5svoqqejohftz2gagjf2al6okkurkt4yqgiv.py
...
#   unsqueeze => unsqueeze
# Graph fragment:
#   %arg0_1 : Tensor "f32[3, 7, 4][28, 4, 1]npu:0" = PlaceHolder[target=arg0_1]
#   %select : Tensor "f32[3, 7][28, 4]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 3), kwargs = {})
#   %unsqueeze : Tensor "f32[3, 7, 1][28, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
#   %add : Tensor "f32[3, 7, 5][35, 5, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%unsqueeze, %arg1_1), kwargs = {})
#   return %buf0
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 105}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'out_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [5, 21, {'divisors': [1, 5]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 5, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 21, 'divisor': 5, 'seed': 2}]},
...
def triton_unk_fused_add_select_unsqueeze_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    x_g_tile1 : tl.constexpr = (21) if (21) < (x_g_rem1) else (x_g_rem1)
    x0numel : tl.constexpr = 5
    x1numel : tl.constexpr = 21
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + (3 + 4 * x1) + _es_lane0, x1mask & ((3 + 4 * x1 + _es_lane0 >= 0) & (3 + 4 * x1 + _es_lane0 < 84)), eviction_policy='evict_last')
        tmp0 = extract_slice(_es_full0, [0, 0], [_es_full0.shape[0], 1], [1, 1])
        tl.store(out_ptr0 + (x0 + 5*x1), tmp0, x0mask & x1mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/attention-slice-boundary-6uy2plji/candidate/inductor-cache/tmpsrdaphv8/ok/coke2a2cmbgoc44epklqqct5q6vsanpq5ntjpm6uwnq5wlkydqb6.py
# Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
# Source node to ATen node mapping:
#   add => add
#   select => select
#   unsqueeze => unsqueeze
# Graph fragment:
#   %buf0 : Tensor "f32[3, 7, 5][35, 5, 1]npu:0" = PlaceHolder[target=buf0]
#   %arg1_1 : Tensor "f32[3, 7, 5][35, 5, 1]npu:0" = PlaceHolder[target=arg1_1]
#   %select : Tensor "f32[3, 7][28, 4]npu:0"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%arg0_1, -1, 3), kwargs = {})
#   %unsqueeze : Tensor "f32[3, 7, 1][28, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%select, -1), kwargs = {})
#   %add : Tensor "f32[3, 7, 5][35, 5, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%unsqueeze, %arg1_1), kwargs = {})
#   return %add
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((3, 7, 5), (35, 5, 1), torch.float32)
            # Topologically Sorted Source Nodes: [select, unsqueeze, add], Original ATen: [aten.select, aten.unsqueeze, aten.add]
...
def get_args():
    ...
    arg0_1 = rand_strided((3, 7, 4), (28, 4, 1), device='npu:0', dtype=torch.float32)
    arg1_1 = rand_strided((3, 7, 5), (35, 5, 1), device='npu:0', dtype=torch.float32)
