...
# kernel path: /home/z50063656/tmp/t106-npu-results/functional-20260915T112026+0800/pattern-22/off/inductor-cache/tmp40xl4uml/k6/ck674jpirmxolj5h635ld7mjvb67g23yjc4qxg5huoxntqwugl3f.py
...
# kernel path: /home/z50063656/tmp/t106-npu-results/functional-20260915T112026+0800/pattern-22/off/inductor-cache/tmp40xl4uml/ji/cji2sv4xvaftgl3viy6aydlrlgchnbkkgq35gd3hlbbrkknt6lwp.py
...
# kernel path: /home/z50063656/tmp/t106-npu-results/functional-20260915T112026+0800/pattern-22/off/inductor-cache/tmp40xl4uml/gs/cgsfnt5yx5cmt4irqyjzjarmb4o6kuausrlktl4vv2fbp2qrddlz.py
# Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
# Source node to ATen node mapping:
#   float_1 => convert_element_type_3
#   masked_score => add
#   score => view_2
#   score_1 => convert_element_type_2
#   softmax => amax, sub
# Graph fragment:
...
#   %add : Tensor "f32[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%view_2, %arg3_1), kwargs = {})
#   %convert_element_type_2 : Tensor "f16[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add, torch.float16), kwargs = {})
#   %convert_element_type_3 : Tensor "f32[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%convert_element_type_2, torch.float32), kwargs = {})
#   %amax : Tensor "f32[2, 8, 4, 1][32, 4, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.amax.default](args = (%convert_element_type_3, [-1], True), kwargs = {})
#   %sub : Tensor "f32[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%convert_element_type_3, %amax), kwargs = {})
#   return %buf3
triton_unk_fused__softmax__to_copy_add_matmul_2 = async_compile.triton('triton_unk_fused__softmax__to_copy_add_matmul_2', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__softmax__to_copy_add_matmul_2', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 8, 'num_reduction': 0, 'npu_num_x_nodes': 3, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused__softmax__to_copy_add_matmul_2(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 256
    x_g_tile0 : tl.constexpr = (4) if (4) < (XBLOCK) else (XBLOCK)
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + (4 * x1 + 128 * x2) + _es_lane0, x1mask & x2mask & ((4 * x1 + 128 * x2 + _es_lane0 >= 0) & (4 * x1 + 128 * x2 + _es_lane0 < 256)), eviction_policy='evict_last').to(tl.float32)
        ...
        tmp2 = extract_slice(_es_full1, [0, 0, 0], [_es_full1.shape[0], _es_full1.shape[1], 1], [1, 1, 1])
        _es_lane2 = tl.arange(0, 4)[None, None, :]
        _es_full2 = tl.load(in_ptr0 + (1 + 4 * x1 + 128 * x2) + _es_lane2, x1mask & x2mask & ((1 + 4 * x1 + 128 * x2 + _es_lane2 >= 0) & (1 + 4 * x1 + 128 * x2 + _es_lane2 < 256)), eviction_policy='evict_last').to(tl.float32)
        ...
        tmp8 = extract_slice(_es_full3, [0, 0, 0], [_es_full3.shape[0], _es_full3.shape[1], 1], [1, 1, 1])
        _es_lane4 = tl.arange(0, 4)[None, None, :]
        _es_full4 = tl.load(in_ptr0 + (2 + 4 * x1 + 128 * x2) + _es_lane4, x1mask & x2mask & ((2 + 4 * x1 + 128 * x2 + _es_lane4 >= 0) & (2 + 4 * x1 + 128 * x2 + _es_lane4 < 256)), eviction_policy='evict_last').to(tl.float32)
        tmp13 = extract_slice(_es_full4, [0, 0, 0], [_es_full4.shape[0], _es_full4.shape[1], 1], [1, 1, 1])
        ...
        tmp15 = extract_slice(_es_full5, [0, 0, 0], [_es_full5.shape[0], _es_full5.shape[1], 1], [1, 1, 1])
        _es_lane6 = tl.arange(0, 4)[None, None, :]
        _es_full6 = tl.load(in_ptr0 + (3 + 4 * x1 + 128 * x2) + _es_lane6, x1mask & x2mask & ((3 + 4 * x1 + 128 * x2 + _es_lane6 >= 0) & (3 + 4 * x1 + 128 * x2 + _es_lane6 < 256)), eviction_policy='evict_last').to(tl.float32)
        tmp20 = extract_slice(_es_full6, [0, 0, 0], [_es_full6.shape[0], _es_full6.shape[1], 1], [1, 1, 1])
        ...
        tmp22 = extract_slice(_es_full7, [0, 0, 0], [_es_full7.shape[0], _es_full7.shape[1], 1], [1, 1, 1])
        ...
        tmp12 = tl.maximum(tmp5, tmp11)
        tmp14 = tmp13.to(tl.float32)
        tmp16 = tmp14 + tmp15
        tmp17 = tmp16.to(tl.float32)
        tmp18 = tmp17.to(tl.float32)
        tmp19 = tl.maximum(tmp12, tmp18)
        tmp21 = tmp20.to(tl.float32)
        tmp23 = tmp21 + tmp22
        tmp24 = tmp23.to(tl.float32)
        tmp25 = tmp24.to(tl.float32)
        tmp26 = tl.maximum(tmp19, tmp25)
        tl.store(out_ptr0 + (x0 + 4*x1 + 128*x2), tmp26, x0mask & x1mask & x2mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t106-npu-results/functional-20260915T112026+0800/pattern-22/off/inductor-cache/tmp40xl4uml/3n/c3notoaaer4vzk4vhuntdmbo4n2k4cyaddi432jykcv6wkbey6w7.py
# Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
# Source node to ATen node mapping:
#   float_1 => convert_element_type_3
#   masked_score => add
...
#   return %sum_1
triton_unk_fused__softmax__to_copy_add_matmul_3 = async_compile.triton('triton_unk_fused__softmax__to_copy_add_matmul_3', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 64}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp16', 'in_ptr1': '*fp32', 'in_ptr2': '*fp32', 'out_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3, 4), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [32, 2, {'divisors': [1, 32]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 32, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 2, 'divisor': 32, 'seed': 2}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__softmax__to_copy_add_matmul_3', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 12, 'num_reduction': 0, 'npu_num_x_nodes': 2, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused__softmax__to_copy_add_matmul_3(in_ptr0, in_ptr1, in_ptr2, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 64
    ...
    for i in range(group_size):
        ...
        _es_sfull0 = tl.load(in_ptr0 + (128 * x1 + 4 * x0offset + _es_slane0), _es_smask0 & x1mask, eviction_policy='evict_last').to(tl.float32)
        ...
        tmp2 = extract_slice(_es_full1, [0, 0], [_es_full1.shape[0], 1], [1, 1])
        ...
        tmp6 = extract_slice(_es_sfull2, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        _es_slane3 = tl.arange(0, 4*real_block_x0)[None, :]
        _es_smask3 = (4*x0offset + _es_slane3) < 4*x0numel
        _es_sfull3 = tl.load(in_ptr0 + (1 + 128 * x1 + 4 * x0offset + _es_slane3), _es_smask3 & x1mask, eviction_policy='evict_last').to(tl.float32)
        tmp9 = extract_slice(_es_sfull3, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        ...
        tmp11 = extract_slice(_es_full4, [0, 0], [_es_full4.shape[0], 1], [1, 1])
        ...
        _es_sfull6 = tl.load(in_ptr0 + (2 + 128 * x1 + 4 * x0offset + _es_slane6), _es_smask6 & x1mask, eviction_policy='evict_last').to(tl.float32)
        tmp19 = extract_slice(_es_sfull6, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        ...
        tmp21 = extract_slice(_es_full7, [0, 0], [_es_full7.shape[0], 1], [1, 1])
        ...
        tmp25 = extract_slice(_es_sfull8, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        _es_slane9 = tl.arange(0, 4*real_block_x0)[None, :]
        _es_smask9 = (4*x0offset + _es_slane9) < 4*x0numel
        _es_sfull9 = tl.load(in_ptr0 + (3 + 128 * x1 + 4 * x0offset + _es_slane9), _es_smask9 & x1mask, eviction_policy='evict_last').to(tl.float32)
        tmp29 = extract_slice(_es_sfull9, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        ...
        tmp31 = extract_slice(_es_full10, [0, 0], [_es_full10.shape[0], 1], [1, 1])
        ...
        tmp35 = extract_slice(_es_sfull11, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        tmp1 = tmp0.to(tl.float32)
        tmp3 = tmp1 + tmp2
        tmp4 = tmp3.to(tl.float32)
        tmp5 = tmp4.to(tl.float32)
        ...
        tmp38 = tmp28 + tmp37
        tl.store(out_ptr0 + (x0 + 32*x1), tmp38, x0mask & x1mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t106-npu-results/functional-20260915T112026+0800/pattern-22/off/inductor-cache/tmp40xl4uml/hs/chsstbw7qo24a3ilvc35st7chh4daiic7gyn4k4zp6jlym6furoy.py
# Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
# Source node to ATen node mapping:
#   float_1 => convert_element_type_3
#   masked_score => add
...
#   return %buf5
triton_unk_fused__softmax__to_copy_add_matmul_4 = async_compile.triton('triton_unk_fused__softmax__to_copy_add_matmul_4', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__softmax__to_copy_add_matmul_4', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 2, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused__softmax__to_copy_add_matmul_4(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 256
    x_g_tile0 : tl.constexpr = (4) if (4) < (XBLOCK) else (XBLOCK)
...
# kernel path: /home/z50063656/tmp/t106-npu-results/functional-20260915T112026+0800/pattern-22/off/inductor-cache/tmp40xl4uml/k3/ck3laafav2gt6qts3maxc5h2mfc6ew4s2pk5sbfclbe3i44or7ek.py
# Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax, type_as_1], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
# Source node to ATen node mapping:
#   float_1 => convert_element_type_3
#   masked_score => add
#   score => view_2
#   score_1 => convert_element_type_2
#   softmax => amax, div, exp, sub
#   type_as_1 => convert_element_type_4
# Graph fragment:
#   %bmm : Tensor "f16[16, 4, 4][16, 4, 1]npu:0" = PlaceHolder[target=bmm]
#   %arg3_1 : Tensor "f32[2, 1, 1, 4][4, 4, 4, 1]npu:0" = PlaceHolder[target=arg3_1]
#   %buf3 : Tensor "f32[2, 8, 4, 4][128, 16, 4, 1]npu:0" = PlaceHolder[target=buf3]
...
#   %exp : Tensor "f32[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.exp.default](args = (%sub,), kwargs = {})
#   %div : Tensor "f32[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%exp, %sum_1), kwargs = {})
#   %convert_element_type_4 : Tensor "f16[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%div, torch.float16), kwargs = {})
#   return %expand_2
triton_unk_fused__softmax__to_copy_add_matmul_5 = async_compile.triton('triton_unk_fused__softmax__to_copy_add_matmul_5', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
def triton_unk_fused__softmax__to_copy_add_matmul_5(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        x2mask = x2index < x2numel
        xmask = x0mask & x1mask & x2mask
        tmp0 = tl.load(in_out_ptr0 + (x0 + 4*x1 + 128*x2), x0mask & x1mask & x2mask).to(tl.float32)
        tmp2 = tl.load(in_ptr0 + (x0 + 4*x2), x0mask & x2mask, eviction_policy='evict_last')
        tmp6 = tl.load(in_ptr1 + (x0 + 4*x1 + 128*x2), x0mask & x1mask & x2mask)
        tmp9 = tl.load(in_ptr2 + (x0 + 4*x1 + 128*x2), x0mask & x1mask & x2mask)
        tmp1 = tmp0.to(tl.float32)
        tmp3 = tmp1 + tmp2
        tmp4 = tmp3.to(tl.float32)
        tmp5 = tmp4.to(tl.float32)
        ...
        tmp10 = (tmp8 / tmp9)
        tmp11 = tmp10.to(tl.float32)
        tl.store(in_out_ptr0 + (x0 + 4*x1 + 128*x2), tmp11, x0mask & x1mask & x2mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            # Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_add_matmul_2.run(buf2, arg3_1, buf3, 256, stream=raw_stream0)
            buf4 = empty_strided_npu((2, 8, 4, 1), (32, 4, 1, 64), torch.float32)
            # Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_add_matmul_3.run(buf2, arg3_1, buf3, buf4, 64, stream=raw_stream0)
            buf5 = empty_strided_npu((2, 8, 4, 4), (128, 16, 4, 1), torch.float32)
            # Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_add_matmul_4.run(buf4, buf5, 256, stream=raw_stream0)
            del buf4
            buf6 = reinterpret_tensor(buf2, (2, 8, 4, 4), (128, 16, 4, 1), 0); del buf2  # reuse
            # Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax, type_as_1], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_add_matmul_5.run(buf6, arg3_1, buf3, buf5, 256, stream=raw_stream0)
            del arg3_1
            del buf3
            del buf5
            ...
            # Topologically Sorted Source Nodes: [score, masked_score, score_1, float_1, softmax, type_as_1, matmul_1, value], Original ATen: [aten.matmul, aten.add, aten._to_copy, aten._softmax, aten.permute]
