...
# kernel path: /home/z50063656/tmp/t105-native-jq9qae5o/adapter/inductor-cache/tmptblc93n7/ie/ciehplp4ngkfey5gioj2zxp7d2274pa64i6ckzds5baexpdexoda.py
...
#   %iota_1 : Tensor "i64[4][1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (4,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: npu:0, requires_grad: False})
#   %unsqueeze_1 : Tensor "i64[4, 1][1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%iota_1, -1), kwargs = {})
#   %sub : Tensor "i64[4, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%unsqueeze, %unsqueeze_1), kwargs = {})
#   %le : Tensor "b8[4, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.le.Scalar](args = (%sub, 0), kwargs = {})
#   return %le
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 8}, 
    filename=__file__,
    triton_meta={'signature': {'out_ptr0': '*i1', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0,), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2, 4, {'divisors': [1, 2]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 4, 'divisor': 2, 'seed': 2}]},
...
# kernel path: /home/z50063656/tmp/t105-native-jq9qae5o/adapter/inductor-cache/tmptblc93n7/rg/crgvbmms22hvx5zxgdczutrban4c56bkp2x62i7ts4btzb2ydwxo.py
# Topologically Sorted Source Nodes: [ones], Original ATen: [aten.ones]
# Source node to ATen node mapping:
#   ones => full_default
# Graph fragment:
#   %full_default : Tensor "b8[4, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([4, 2], True), kwargs = {dtype: torch.bool, layout: torch.strided, device: npu:0, pin_memory: False})
#   return %full_default
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 8}, 
    filename=__file__,
    triton_meta={'signature': {'out_ptr0': '*i1', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0,), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [8, {'divisors': [1]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 8, 'divisor': 1, 'seed': 1}]},
...
# kernel path: /home/z50063656/tmp/t105-native-jq9qae5o/adapter/inductor-cache/tmptblc93n7/m2/cm2fizmbektn3umk2crjpwshzwcg2wpkhgbqkt2kcmjm2ym46x6w.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.transpose, aten.eq, aten.view, aten.expand, aten.logical_not, npu.npu_fusion_attention_v3]
# Source node to ATen node mapping:
#    => eq_scalar, expand_default, logical_not_default, npu_fusion_attention_v3_default, permute_default, permute_default_1, permute_default_2, view_default
# Graph fragment:
#   %logical_and : Tensor "b8[4, 2][2, 1]npu:0" = PlaceHolder[target=logical_and]
...
#   %eq_scalar : Tensor "b8[4, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%logical_and, 1), kwargs = {})
#   %view_default : Tensor "b8[4, 1, 1, 2][2, 2, 2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%eq_scalar, [4, 1, 1, 2]), kwargs = {})
#   %expand_default : Tensor "b8[4, 16, 2, 2][2, 0, 0, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%view_default, [4, 16, 2, 2]), kwargs = {})
#   %logical_not_default : Tensor "b8[4, 16, 2, 2][64, 4, 2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.logical_not.default](args = (%expand_default,), kwargs = {})
#   %npu_fusion_attention_v3_default : [num_users=1] = call_function[target=torch.ops.npu.npu_fusion_attention_v3.default](args = (%permute_default, %permute_default_1, %permute_default_2, 16, BNSD, None, None, %logical_not_default, 0.17677669529663687, 1.0, 2147483647, 2147483647, 2), kwargs = {})
#   return %buf4
triton_unk_fused_eq_expand_logical_not_npu_fusion_attention_v3_transpose_view_2 = async_compile.triton('triton_unk_fused_eq_expand_logical_not_npu_fusion_attention_v3_transpose_view_2', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 256}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i1', 'out_ptr0': '*i1', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2, 32, 4, {'divisors': [1, 2, 64]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 32, 'divisor': 2, 'seed': 2}, {'name': 'x2', 'length': 4, 'divisor': 64, 'seed': 2}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_eq_expand_logical_not_npu_fusion_attention_v3_transpose_view_2', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 3, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_eq_expand_logical_not_npu_fusion_attention_v3_transpose_view_2(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 256
    x_g_tile0 : tl.constexpr = (2) if (2) < (XBLOCK) else (XBLOCK)
    ...
    for i in range(group_size):
        ...
        tmp0 = tl.load(in_ptr0 + (x0 + 2 * x2), x0mask & x2mask, eviction_policy='evict_last') != 0
        tmp1 = tmp0.to(tl.int32)
        tmp2 = tl.full([1], 1, tl.int32)
        tmp3 = tmp1 == tmp2
        tmp4 = tmp3 == 0
        tl.store(out_ptr0 + (x0 + 2*x1 + 64*x2), tmp4, x0mask & x1mask & x2mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf1 = empty_strided_npu((4, 2), (2, 1), torch.bool)
            # Topologically Sorted Source Nodes: [tril], Original ATen: [aten.tril]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_tril_0.run(buf1, 8, stream=raw_stream0)
            buf2 = empty_strided_npu((4, 2), (2, 1), torch.bool)
            # Topologically Sorted Source Nodes: [ones], Original ATen: [aten.ones]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_ones_1.run(buf2, 8, stream=raw_stream0)
            ...
            del buf1
            del buf2
            buf4 = empty_strided_npu((4, 16, 2, 2), (64, 4, 2, 1), torch.bool)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.transpose, aten.eq, aten.view, aten.expand, aten.logical_not, npu.npu_fusion_attention_v3]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_eq_expand_logical_not_npu_fusion_attention_v3_transpose_view_2.run(buf3, buf4, 256, stream=raw_stream0)
            del buf3
            arg1_1 = copy_if_misaligned(arg1_1)
            arg0_1 = copy_if_misaligned(arg0_1)
            arg2_1 = copy_if_misaligned(arg2_1)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.transpose, aten.eq, aten.view, aten.expand, aten.logical_not, npu.npu_fusion_attention_v3]
            # [Provenance debug handles] torch.ops.npu.npu_fusion_attention_v3.default:2
            buf5 = torch.ops.npu.npu_fusion_attention_v3.default(reinterpret_tensor(arg1_1, (4, 16, 2, 32), (1024, 32, 512, 1), 0), reinterpret_tensor(arg0_1, (4, 16, 2, 32), (1024, 32, 512, 1), 0), reinterpret_tensor(arg2_1, (4, 16, 2, 32), (1024, 32, 512, 1), 0), 16, 'BNSD', None, None, buf4, 0.17677669529663687, 1.0, 2147483647, 2147483647, 2, prefix=None, actual_seq_qlen=None, actual_seq_kvlen=None, sparse_mode=0, gen_mask_parallel=True, sync=False, softmax_layout='', sink=None)
            del arg0_1
            del arg1_1
            del arg2_1
            del buf4
            buf6 = buf5[0]
            assert_alignment(buf6, 16, 'torch.ops.npu.npu_fusion_attention_v3.default')
            del buf5
        return (buf6, )
