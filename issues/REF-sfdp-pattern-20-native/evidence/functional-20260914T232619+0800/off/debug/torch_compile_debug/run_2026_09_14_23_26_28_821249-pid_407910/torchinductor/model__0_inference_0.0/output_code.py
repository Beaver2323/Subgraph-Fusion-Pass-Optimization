...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/6v/c6vlqzzj6vsohqntj2d2cbsyduqg23h5ybrjvwkibmtkpydropow.py
# Topologically Sorted Source Nodes: [q, q_1, scores], Original ATen: [aten.permute, aten.div, aten.matmul]
...
#   %div : Tensor "f16[2, 8, 4, 16][512, 16, 128, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%permute, 0.66666), kwargs = {})
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_div_matmul_permute_0', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 4, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_div_matmul_permute_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    ...
    for i in range(group_size):
        ...
        tmp1 = tl.full([1], 1.5000150001500014, tl.float32)
        tmp2 = tmp0 * tmp1
        tl.store(out_ptr0 + (x0 + 16*x1 + 64*x2 + 512*x3), tmp2, x0mask & x1mask & x2mask & x3mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/vy/cvy7lkqhjtituenoxv2hel4ww4vu7wkfg5pj2uinra6f4no2udgo.py
...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/cu/ccujpvxicyyf2pn6wkorz4isn2puiocp4mlkben4z3vq4lbvqsnw.py
# Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
# Source node to ATen node mapping:
#   attn_mask => expand_2
#   eq => eq
...
#   view => view_3
# Graph fragment:
#   %arg3_1 : Tensor "f16[2, 4][4, 1]npu:0" = PlaceHolder[target=arg3_1]
#   %bmm : Tensor "f16[16, 4, 4][16, 4, 1]npu:0" = PlaceHolder[target=bmm]
#   %eq : Tensor "b8[2, 4][4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%arg3_1, 0), kwargs = {})
...
#   %where : Tensor "f16[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%expand_2, %full_default, %view_2), kwargs = {})
...
#   return %buf3
triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_2 = async_compile.triton('triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_2', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
def triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_2(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + 4 * x2 + _es_lane0, x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp0 = extract_slice(_es_full0, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane1 = tl.arange(0, 4)[None, None, :]
        _es_full1 = tl.load(in_ptr1 + (4 * x1 + 128 * x2) + _es_lane1, x1mask & x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp3 = extract_slice(_es_full1, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane2 = tl.arange(0, 4)[None, None, :]
        _es_full2 = tl.load(in_ptr0 + (1 + 4 * x2) + _es_lane2, x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp7 = extract_slice(_es_full2, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane3 = tl.arange(0, 4)[None, None, :]
        _es_full3 = tl.load(in_ptr1 + (1 + 4 * x1 + 128 * x2) + _es_lane3, x1mask & x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp9 = extract_slice(_es_full3, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane4 = tl.arange(0, 4)[None, None, :]
        _es_full4 = tl.load(in_ptr0 + (2 + 4 * x2) + _es_lane4, x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp13 = extract_slice(_es_full4, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane5 = tl.arange(0, 4)[None, None, :]
        _es_full5 = tl.load(in_ptr1 + (2 + 4 * x1 + 128 * x2) + _es_lane5, x1mask & x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp15 = extract_slice(_es_full5, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane6 = tl.arange(0, 4)[None, None, :]
        _es_full6 = tl.load(in_ptr0 + (3 + 4 * x2) + _es_lane6, x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp19 = extract_slice(_es_full6, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
        _es_lane7 = tl.arange(0, 4)[None, None, :]
        _es_full7 = tl.load(in_ptr1 + (3 + 4 * x1 + 128 * x2) + _es_lane7, x1mask & x2mask, eviction_policy='evict_last').to(tl.float32)
        tmp21 = extract_slice(_es_full7, [0, 0, 0], [real_block_x2, real_block_x1, 1], [1, 1, 1])
...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/mp/cmp57nqcnbsdx3lhzjpdeczm4pnlg5ecoy3is5hiswq7ah2tm6gq.py
# Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
# Source node to ATen node mapping:
#   attn_mask => expand_2
#   eq => eq
...
#   return %sum_1
triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_3 = async_compile.triton('triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_3', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 64}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp16', 'in_ptr1': '*fp16', 'in_ptr2': '*fp32', 'out_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3, 4), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [32, 2, {'divisors': [1, 32]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 32, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 2, 'divisor': 32, 'seed': 2}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_3', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 12, 'num_reduction': 0, 'npu_num_x_nodes': 2, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_3(in_ptr0, in_ptr1, in_ptr2, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 64
    ...
    for i in range(group_size):
        ...
        tmp7 = extract_slice(_es_sfull2, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        _es_lane3 = tl.arange(0, 4)[None, :]
        _es_full3 = tl.load(in_ptr0 + (1 + 4 * x1) + _es_lane3, x1mask, eviction_policy='evict_last').to(tl.float32)
        tmp10 = extract_slice(_es_full3, [0, 0], [real_block_x1, 1], [1, 1])
        ...
        tmp12 = extract_slice(_es_sfull4, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        ...
        tmp15 = extract_slice(_es_sfull5, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        _es_lane6 = tl.arange(0, 4)[None, :]
        _es_full6 = tl.load(in_ptr0 + (2 + 4 * x1) + _es_lane6, x1mask, eviction_policy='evict_last').to(tl.float32)
        tmp19 = extract_slice(_es_full6, [0, 0], [real_block_x1, 1], [1, 1])
        ...
        tmp21 = extract_slice(_es_sfull7, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        ...
        tmp24 = extract_slice(_es_sfull8, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        _es_lane9 = tl.arange(0, 4)[None, :]
        _es_full9 = tl.load(in_ptr0 + (3 + 4 * x1) + _es_lane9, x1mask, eviction_policy='evict_last').to(tl.float32)
        tmp28 = extract_slice(_es_full9, [0, 0], [real_block_x1, 1], [1, 1])
        ...
        tmp30 = extract_slice(_es_sfull10, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        ...
        tmp33 = extract_slice(_es_sfull11, [0, 0], [real_block_x1, real_block_x0], [1, 4])
        tmp1 = tl.full([1], 0.0, tl.float32)
        tmp2 = tmp0 == tmp1
        tmp4 = tl.full([1], float("-inf"), tl.float32)
        tmp5 = tl.where(tmp2, tmp4, tmp3)
        ...
        tmp11 = tmp10 == tmp1
        tmp13 = tl.where(tmp11, tmp4, tmp12)
        ...
        tmp27 = tmp18 + tmp26
        tmp29 = tmp28 == tmp1
        tmp31 = tl.where(tmp29, tmp4, tmp30)
...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/cy/ccypgnivdlzrsbgyowwud7ryhiirrsmu3cnjuhkn6kvraqygzof3.py
# Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
# Source node to ATen node mapping:
#   attn_mask => expand_2
#   eq => eq
...
#   return %buf5
triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_4 = async_compile.triton('triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_4', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_4', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 2, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_4(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 256
    x_g_tile0 : tl.constexpr = (4) if (4) < (XBLOCK) else (XBLOCK)
...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/4j/c4ji6qbz4rpuyn46p5smlp5yfimgb6kyogcoqwlnvozf6cny2idv.py
# Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
# Source node to ATen node mapping:
#   attn_mask => expand_2
#   eq => eq
...
#   return %expand_3
triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_5 = async_compile.triton('triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_5', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232619+0800/pattern-20/off/inductor-cache/tmptgpcvsye/gq/cgq7yi4hypp7a6zwhslkgzlcqfh2redlcwd5eykuns27cz2g3uav.py
...
#   %permute_2 : Tensor "f16[2, 8, 4, 16][512, 16, 128, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%arg2_1, [0, 2, 1, 3]), kwargs = {})
#   %clone_2 : Tensor "f16[2, 8, 4, 16][512, 64, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.clone.default](args = (%expand_4,), kwargs = {memory_format: torch.contiguous_format})
#   return %clone_2
triton_unk_fused_matmul_permute_6 = async_compile.triton('triton_unk_fused_matmul_permute_6', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused_div_matmul_permute_0.run(arg0_1, buf0, 1024, stream=raw_stream0)
            del arg0_1
            ...
            # Topologically Sorted Source Nodes: [q, q_1, scores, k, transpose], Original ATen: [aten.permute, aten.div, aten.matmul, aten.transpose]
            ...
            # Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_2.run(arg3_1, buf2, buf3, 256, stream=raw_stream0)
            buf4 = empty_strided_npu((2, 8, 4, 1), (32, 4, 1, 64), torch.float32)
            # Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_3.run(arg3_1, buf2, buf3, buf4, 64, stream=raw_stream0)
            buf5 = empty_strided_npu((2, 8, 4, 4), (128, 16, 4, 1), torch.float32)
            # Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_4.run(buf4, buf5, 256, stream=raw_stream0)
            del buf4
            buf6 = reinterpret_tensor(buf2, (2, 8, 4, 4), (128, 16, 4, 1), 0); del buf2  # reuse
            # Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_eq_expand_full_masked_fill_matmul_view_5.run(buf6, arg3_1, buf3, buf5, 256, stream=raw_stream0)
            del arg3_1
            del buf3
            del buf5
            ...
            # Topologically Sorted Source Nodes: [v, matmul_1], Original ATen: [aten.permute, aten.matmul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_matmul_permute_6.run(arg2_1, buf7, 1024, stream=raw_stream0)
            del arg2_1
            buf8 = empty_strided_npu((16, 4, 16), (64, 16, 1), torch.float16)
            # Topologically Sorted Source Nodes: [eq, view, attn_mask, fill_value, scores, masked_fill, softmax, matmul_1, v], Original ATen: [aten.eq, aten.view, aten.expand, aten.full, aten.matmul, aten.masked_fill, aten._softmax, aten.permute]
            ...
            del buf6
            del buf7
        return (reinterpret_tensor(buf8, (2, 8, 4, 16), (512, 64, 16, 1), 0), )
...
def get_args():
    ...
    arg3_1 = rand_strided((2, 4), (4, 1), device='npu:0', dtype=torch.float16)
