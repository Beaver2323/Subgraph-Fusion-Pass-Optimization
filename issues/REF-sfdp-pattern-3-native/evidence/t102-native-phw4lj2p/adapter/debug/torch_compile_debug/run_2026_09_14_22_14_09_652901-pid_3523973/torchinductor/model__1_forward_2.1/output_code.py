...
# kernel path: /home/z50063656/tmp/t102-native-phw4lj2p/adapter/inductor-cache/tmp4mj5qcb1/rn/crnuq5nx6d2kc62wupb655najrjatbqpvwnw2zwz2mb35azhpj3f.py
...
#   %div_tensor : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=4] = call_function[target=torch.ops.aten.div.Tensor](args = (%view_2, 3.0), kwargs = {})
#   %eq_tensor : Tensor "b8[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.eq.Tensor](args = (%div_tensor, %div_tensor), kwargs = {})
#   %abs_default : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.abs.default](args = (%div_tensor,), kwargs = {})
...
#   %mul_tensor_1 : Tensor "b8[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%eq_tensor, %ne_scalar), kwargs = {})
#   %logical_not_default : Tensor "b8[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.logical_not.default](args = (%mul_tensor_1,), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t102-native-phw4lj2p/adapter/inductor-cache/tmp4mj5qcb1/iz/cizkzvevdtlxz2tfslbd4mwqllvzxcj4xunamblof6p7q3nj4mdg.py
...
#   %view_2 : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.reshape.default](args = (%bmm, [4, 2, 16, 16]), kwargs = {})
#   %div_tensor : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=4] = call_function[target=torch.ops.aten.div.Tensor](args = (%view_2, 3.0), kwargs = {})
#   %mul_tensor : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view_2, 1), kwargs = {})
#   %amax_default : Tensor "f32[4, 2, 16, 1][32, 16, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.amax.default](args = (%mul_tensor, [-1], True), kwargs = {})
#   %sub_tensor : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%mul_tensor, %amax_default), kwargs = {})
#   %div_tensor_1 : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%sub_tensor, 3.0), kwargs = {})
#   %amax_default_1 : Tensor "f32[4, 2, 16, 1][32, 16, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.amax.default](args = (%div_tensor, [-1], True), kwargs = {})
#   %sub_tensor_1 : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%div_tensor, %amax_default_1), kwargs = {})
...
#   %where_self : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%logical_not_default_1, %div_tensor_1, %sub_tensor_1), kwargs = {})
...
def triton_unk_fused__softmax_all_amax_div_matmul_mul_sub_1(in_out_ptr0, in_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            _tmp4 = tmp5
            tmp6 = tl.full([1, 1], 0.3333333333333333, tl.float32)
            tmp7 = tmp0 * tmp6
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp15 = tmp13 * tmp14
            tmp16 = tmp15 - tmp4
            tmp17 = tl.full([1, 1], 0.3333333333333333, tl.float32)
            tmp18 = tmp16 * tmp17
            tmp19 = tmp13 * tmp17
            tmp20 = tmp19 - tmp9
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp29 = tmp27 * tmp28
            tmp30 = tmp29 - tmp4
            tmp31 = tl.full([1, 1], 0.3333333333333333, tl.float32)
            tmp32 = tmp30 * tmp31
            tmp33 = tmp27 * tmp31
            tmp34 = tmp33 - tmp9
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused_all_div_isfinite_matmul_0.run(buf0, buf3, 2048, stream=raw_stream0)
            # Topologically Sorted Source Nodes: [matmul, ], Original ATen: [aten.matmul, aten.div, aten.isfinite, aten.all]
            ...
            # Topologically Sorted Source Nodes: [matmul, , softmax], Original ATen: [aten.matmul, aten.div, aten.mul, aten.amax, aten.sub, aten.all, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_all_amax_div_matmul_mul_sub_1.run(buf6, buf4, 128, 16, stream=raw_stream0)
            del buf4
            ...
            buf7 = torch.ops.npu._npu_dropout.default(buf6, 0.4)
            buf8 = buf7[0]
