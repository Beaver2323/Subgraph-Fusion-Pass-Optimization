...
test_sdpa_rewriter_3_gpu → _check_common → torch.compile(fullgraph=True)
...
| NPU 修复前 | 原训练合同 FAIL | [原 result](evidence/t102-native-phw4lj2p/adapter/result.json) |
| 隔离共同候选 | 原方法 PASS，不算安装态 | [候选 result](evidence/t102-native-5rrs8ipf/adapter/result.json) |
| 安装态无候选新进程 | tests=1、skip=0、本编号改写 2 次 | [新 result](evidence/t102-native-9qcchep4/adapter/result.json) |
...
/home/z50063656/envs/Pass/bin/python /home/z50063656/Pass/Subgraph-Fusion-Pass-Optimization/issues/REF-sfdp-pattern-3-native/npu_adapter.py --artifact-dir /home/z50063656/tmp/t102-native-9qcchep4/adapter
```

[完整父命令/哈希/退出码及 stdout/stderr](adapter_runs/native-20260915T192044-cuk688jy/)。返回码 0，`Ran 1 test` / `OK`，不含候选开关。
...
同 NPU 5、六个新进程，顺序 OFF1/ON1/ON2/OFF2/OFF3/ON3；每臂预热10次、host及Event各100样本。汇总取三轮各臂p50/p99的中位数，原样本、波动和compile_ms保留。compile_ms起点位于先行lazy_init之后，不是包含训练注册trace的完整冷启动成本。[功能门禁](../../results/current/T-102/performance_gates/pattern-3-executed.json)、[完整验签包](../../results/current/T-102/attention_bundles/pattern-3.json)、[时延原始记录](../../results/current/T-102/performance_summary.json)。
