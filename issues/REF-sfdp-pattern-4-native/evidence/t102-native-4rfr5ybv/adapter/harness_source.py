...
REVIEWED_MATH_PATTERNS = (14, 21, 22, 24)
...
def reviewed_math_codegen(code, observations, pattern):
    """只承接逐例人工审查的SDPA数学展开；不是任意缺失FA符号的通行证。"""
    if pattern not in REVIEWED_MATH_PATTERNS:
        return False
    calls = [ast.unparse(node.func) for node in ast.walk(ast.parse(code)) if isinstance(node, ast.Call)]
    target_changed = any(row.get('target', '').startswith(f'_sfdp_pattern_{pattern}_')
...
def main(pattern):
    ...
    parser.add_argument('--registration-module', type=Path)
    parser.add_argument('--select-slice-candidate', action='store_true')
    parser.add_argument('--allow-reviewed-sdpa-math', action='store_true')
    args = parser.parse_args()
    if args.registration_module and (not args.registration_candidate or pattern not in (1,2,3,4,5)):
        parser.error('产品化候选仅用于显式隔离的 T-102 注册复验')
    if pattern not in (*range(1, 16), *range(18, 25), 28, 30):
        parser.error('本入口只执行已取得精确 GPU 归因的编号')
    if args.allow_reviewed_sdpa_math and pattern not in REVIEWED_MATH_PATTERNS:
        parser.error('数学展开的代码断言适配仅逐例审查了pattern 14/21/22/24')
    if args.select_slice_candidate and (pattern not in (21,22,23,24) or args.registration_candidate):
        parser.error('codegen候选仅用于22号原例及21/23/24近邻，不与注册候选混用')
    ...
    snapshot = (out/'harness_source.py').read_text()
    # traceback按本轮执行器快照回显；mtime=None阻止linecache换成后来工作树的行文。
    linecache.cache[__file__] = (len(snapshot), None, snapshot.splitlines(keepends=True), __file__)
    ...
    candidate_records = []
    codegen_candidate = None
    if args.select_slice_candidate:
        from attention_select_slice_candidate import install as install_codegen_candidate
        codegen_candidate = install_codegen_candidate(out/'codegen-candidate')
    if args.registration_candidate:
        if args.registration_module:
            source = args.registration_module.resolve(strict=True)
            (out/'candidate_source.py').write_bytes(source.read_bytes())
            candidate_spec = importlib.util.spec_from_file_location('sfdp_product_candidate', out/'candidate_source.py')
            candidate_module = importlib.util.module_from_spec(candidate_spec)
            candidate_spec.loader.exec_module(candidate_module)
            candidate_module.install_sfdp_training_patterns()
            candidate_records = [{'mode': 'reviewed-1-to-5-product-module', 'product_gate_bypassed': False}]
        else:
        ...
        def assertIn(self, member, container, *pos, **kw):
            ...
            if not accepted and args.allow_reviewed_sdpa_math:
                ...
                self.assertTrue(reviewed_math_codegen(container, observed, pattern),
                                '必须同时有本编号真实改图及已审查的NPU BMM/safe-softmax数学展开')
    ...
    record = dict(generated_at=datetime.now().astimezone().isoformat(), started_at=started, pid=os.getpid(),
                  ...
                  isolated_codegen_candidate=args.select_slice_candidate,
                  codegen_candidate=codegen_candidate,
