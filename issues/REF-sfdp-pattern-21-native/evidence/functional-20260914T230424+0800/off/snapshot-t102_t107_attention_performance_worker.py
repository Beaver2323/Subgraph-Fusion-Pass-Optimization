...
def select_registration(candidates, pattern):
    training = pattern in LOW_DROPOUT_TRAINING_PATTERNS
    ...
    eligible.sort(key=lambda item: ('_half' not in item[0], 'mask_fp32' in item[0],
...
def main() -> None:
    ...
    for index, value in enumerate(registration['example_inputs']):
        ...
        inputs_list.append(initialized)
    inputs = tuple(inputs_list)
    input_spec = [{'shape':list(x.shape),'stride':list(x.stride()),'dtype':str(x.dtype),
                   'requires_grad':x.requires_grad} for x in inputs]
    if gate:
    ...
    record = {
        ...
        "input_initialization": "fixed-seed-normal-std0.25; preserve-shape-stride-dtype; preserve-scalar-constants",
        "argument_initialization": [input_initialization_policy(args.pattern, i) for i in range(len(inputs))],
