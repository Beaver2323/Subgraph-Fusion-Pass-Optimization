...
# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/zi/cziymxfd7mjgxqx7bzblzcwzay5w734x54jqr54ezjd5szgmtiw3.py
...
# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/2f/c2fzyqjal4tidfupi52q5suwdqts6rqyn2rgaorj3dz6bm4wrnlv.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.permute, aten.transpose, aten.mul, aten.matmul]
# Source node to ATen node mapping:
...
#   %permute_default_1 : Tensor "f32[4, 16, 2, 32][1024, 32, 512, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%arg1_1, [0, 2, 1, 3]), kwargs = {})
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid2D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_matmul_mul_permute_transpose_1', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 3, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_matmul_mul_permute_transpose_1(in_ptr0, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    ynumel = 2048
    xnumel = 2
...
# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/2f/c2fspjnql5qc5cuclrkkoexdnqmj5sburucsalez2irs2pumthvy.py
...
#   %arg0_1 : Tensor "f32[1, 1, 1, 2][2, 2, 2, 1]npu:0" = PlaceHolder[target=arg0_1]
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 256}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'out_ptr0': '*i1', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2, 128, {'divisors': [1, 2]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 128, 'divisor': 2, 'seed': 2}]},
...
def triton_unk_fused__safe_softmax_add_matmul_2(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        x1mask = x1index < x1numel
        xmask = x0mask & x1mask
        tmp0 = tl.load(in_ptr0 + (x0 + 2*x1), x0mask & x1mask)
        tmp1 = tl.load(in_ptr1 + (x0), x0mask, eviction_policy='evict_last')
        tmp2 = tmp0 + tmp1
        tmp3 = tl.full([1], float("-inf"), tl.float32)
        tmp4 = tmp2 == tmp3
        tmp5 = tmp4 == 0
        tl.store(out_ptr0 + (x0 + 2*x1), tmp5, x0mask & x1mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/w3/cw3sadlqly6mmqwsfbkw5cod5exw33gjhm23udfjgqv3srta52ua.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.add, aten._safe_softmax]
# Source node to ATen node mapping:
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 256}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'out_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2, 64, 2, {'divisors': [1, 2, 128]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2, 'divisor': 1, 'seed': 1}, {'name': 'x2', 'length': 64, 'divisor': 2, 'seed': 2}, {'name': 'x3', 'length': 2, 'divisor': 128, 'seed': 2}]},
...
def triton_unk_fused__safe_softmax_add_matmul_3(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        _es_full0 = tl.load(in_ptr0 + (2 * x2 + 128 * x3) + _es_lane0, x2mask & x3mask & ((2 * x2 + 128 * x3 + _es_lane0 >= 0) & (2 * x2 + 128 * x3 + _es_lane0 < 256)), eviction_policy='evict_last')
        tmp0 = extract_slice(_es_full0, [0, 0, 0], [_es_full0.shape[0], _es_full0.shape[1], 1], [1, 1, 1])
        tmp1 = tl.load(in_ptr1 + (0))
        tmp2 = tmp1
        _es_lane1 = tl.arange(0, 2)[None, None, :]
        _es_full1 = tl.load(in_ptr0 + (1 + 2 * x2 + 128 * x3) + _es_lane1, x2mask & x3mask & ((1 + 2 * x2 + 128 * x3 + _es_lane1 >= 0) & (1 + 2 * x2 + 128 * x3 + _es_lane1 < 256)), eviction_policy='evict_last')
        tmp4 = extract_slice(_es_full1, [0, 0, 0], [_es_full1.shape[0], _es_full1.shape[1], 1], [1, 1, 1])
        tmp5 = tl.load(in_ptr1 + (1))
        tmp6 = tmp5
        tmp3 = tmp0 + tmp2
        tmp7 = tmp4 + tmp6
        tmp8 = tl.maximum(tmp3, tmp7)
        tl.store(out_ptr0 + (x0 + 2*x2 + 128*x3), tmp8, x0mask & x2mask & x3mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/zj/czjhvimcii44zsmih6l5zmiszi44lgzfa4lvqwe4x6yoltdkapsm.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.add, aten._safe_softmax]
# Source node to ATen node mapping:
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 256}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'in_ptr2': '*fp32', 'out_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3, 4), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2, 64, 2, {'divisors': [1, 2, 128]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2, 'divisor': 1, 'seed': 1}, {'name': 'x2', 'length': 64, 'divisor': 2, 'seed': 2}, {'name': 'x3', 'length': 2, 'divisor': 128, 'seed': 2}]},
...
def triton_unk_fused__safe_softmax_add_matmul_4(in_ptr0, in_ptr1, in_ptr2, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        tmp2 = tmp1
        _es_lane1 = tl.arange(0, 2)[None, None, :]
        _es_full1 = tl.load(in_ptr2 + (2 * x2 + 128 * x3) + _es_lane1, x2mask & x3mask & ((2 * x2 + 128 * x3 + _es_lane1 >= 0) & (2 * x2 + 128 * x3 + _es_lane1 < 256)), eviction_policy='evict_last')
        tmp4 = extract_slice(_es_full1, [0, 0, 0], [_es_full1.shape[0], _es_full1.shape[1], 1], [1, 1, 1])
        _es_lane2 = tl.arange(0, 2)[None, None, :]
        _es_full2 = tl.load(in_ptr0 + (1 + 2 * x2 + 128 * x3) + _es_lane2, x2mask & x3mask & ((1 + 2 * x2 + 128 * x3 + _es_lane2 >= 0) & (1 + 2 * x2 + 128 * x3 + _es_lane2 < 256)), eviction_policy='evict_last')
        tmp7 = extract_slice(_es_full2, [0, 0, 0], [_es_full2.shape[0], _es_full2.shape[1], 1], [1, 1, 1])
        tmp8 = tl.load(in_ptr1 + (1))
        tmp9 = tmp8
        _es_lane3 = tl.arange(0, 2)[None, None, :]
        _es_full3 = tl.load(in_ptr2 + (1 + 2 * x2 + 128 * x3) + _es_lane3, x2mask & x3mask & ((1 + 2 * x2 + 128 * x3 + _es_lane3 >= 0) & (1 + 2 * x2 + 128 * x3 + _es_lane3 < 256)), eviction_policy='evict_last')
        tmp11 = extract_slice(_es_full3, [0, 0, 0], [_es_full3.shape[0], _es_full3.shape[1], 1], [1, 1, 1])
        tmp3 = tmp0 + tmp2
        tmp5 = tmp3 - tmp4
        ...
        tmp14 = tmp6 + tmp13
        tl.store(out_ptr0 + (x0 + 2*x2 + 128*x3), tmp14, x0mask & x2mask & x3mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/6f/c6fixvxvqkhex5n6yukkh2mjked4tzytrqfdeerivaupncfrwyiz.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.add, aten._safe_softmax]
# Source node to ATen node mapping:
...
# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/jd/cjd4gquv3luhv573duse2zvunqm2qycjcfada33fwqkq3g7pbhvy.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.add, aten._safe_softmax]
# Source node to ATen node mapping:
#    => add_tensor, amax_default, div_tensor, exp_default, full_default, logical_not_default_1, sub_tensor, sum_dim_int_list, view_default_2, where_self
# Graph fragment:
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 256}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*i1', 'in_ptr1': '*fp32', 'in_ptr2': '*fp32', 'in_ptr3': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3, 4, 5), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2, 128, {'divisors': [1, 2]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 128, 'divisor': 2, 'seed': 2}]},
...
def triton_unk_fused__safe_softmax_add_matmul_6(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        x1mask = x1index < x1numel
        xmask = x0mask & x1mask
        tmp0 = tl.load(in_ptr0 + (x0 + 2 * x1), x0mask & x1mask) != 0
        tmp1 = tl.load(in_out_ptr0 + (x0 + 2*x1), x0mask & x1mask)
        tmp2 = tl.load(in_ptr1 + (x0), x0mask, eviction_policy='evict_last')
        tmp4 = tl.load(in_ptr2 + (x0 + 2*x1), x0mask & x1mask)
        tmp7 = tl.load(in_ptr3 + (x0 + 2*x1), x0mask & x1mask)
        tmp3 = tmp1 + tmp2
        tmp5 = tmp3 - tmp4
        tmp6 = libdevice.exp(tmp5)
        tmp8 = (tmp6 / tmp7)
        ...
        tl.store(in_out_ptr0 + (x0 + 2*x1), tmp10, x0mask & x1mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t106-native-i2427mng/adapter/inductor-cache/tmpo81p32sq/vr/cvrufcuwz23u6ianr3febkkgoxt6ghvpyyb2gy3wu65zznqg7myr.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.permute, aten.matmul]
# Source node to ATen node mapping:
#    => clone_default_2, permute_default_2
# Graph fragment:
#   %arg3_1 : Tensor "f32[4, 2, 16, 32][1024, 512, 32, 1]npu:0" = PlaceHolder[target=arg3_1]
#   %permute_default_2 : Tensor "f32[4, 16, 2, 32][1024, 32, 512, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%arg3_1, [0, 2, 1, 3]), kwargs = {})
...
triton_unk_fused_matmul_permute_7 = async_compile.triton('triton_unk_fused_matmul_permute_7', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.permute, aten.transpose, aten.mul, aten.matmul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_matmul_mul_permute_transpose_1.run(arg1_1, buf1, 2048, 2, stream=raw_stream0)
            del arg1_1
            ...
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.permute, aten.matmul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_matmul_permute_7.run(arg3_1, buf9, 4096, stream=raw_stream0)
            del arg3_1
            buf10 = empty_strided_npu((64, 2, 32), (64, 32, 1), torch.float32)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.add, aten._safe_softmax, aten.permute]
            ...
            del buf8
            del buf9
        return (reinterpret_tensor(buf10, (4, 16, 2, 32), (1024, 64, 32, 1), 0), )
...
def get_args():
    ...
    arg0_1 = rand_strided((1, 1, 1, 2), (2, 2, 2, 1), device='npu:0', dtype=torch.float32)
