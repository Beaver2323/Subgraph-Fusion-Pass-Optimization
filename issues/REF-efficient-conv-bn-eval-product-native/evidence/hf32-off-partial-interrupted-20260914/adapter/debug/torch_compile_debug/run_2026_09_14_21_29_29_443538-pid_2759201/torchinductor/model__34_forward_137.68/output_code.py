...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpkvt14q2h/r6/cr6zjji5v3ekplgzas4whtgfn33txa46muon2icon2neto72tvo7.py
...
#   %view : Tensor "f32[32, 1, 1][1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%rsqrt, [-1, 1, 1]), kwargs = {})
...
#   %mul : Tensor "f32[32, 1, 1][1, 1, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view_1, %view), kwargs = {})
#   %mul_1 : Tensor "f32[32, 3, 3][9, 3, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_5, %mul), kwargs = {})
#   return %buf0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpkvt14q2h/l2/cl24nw77vjtesqinpskazmbqcfzs6xyz7ifpicymrnjttmst234x.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, mul, mul_1, rsqrt, view, view_1
# Graph fragment:
#   %primals_5 : Tensor "f32[32, 3, 3][9, 3, 1]npu:0" = PlaceHolder[target=primals_5]
#   %buf0 : Tensor "f32[32, 3, 3][9, 3, 1]npu:0" = PlaceHolder[target=buf0]
#   %add : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_2, 1e-05), kwargs = {})
#   %rsqrt : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add,), kwargs = {})
#   %view : Tensor "f32[32, 1, 1][1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%rsqrt, [-1, 1, 1]), kwargs = {})
#   %view_1 : Tensor "f32[32, 1, 1][1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%primals_4, [32, 1, 1]), kwargs = {})
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 288}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [288, {'divisors': [1]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 288, 'divisor': 1, 'seed': 1}]},
...
def triton_unk_fused_add_mul_rsqrt_view_1(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    group_id = tl.program_id(0)
    xnumel = 288
    x_g_tile0 : tl.constexpr = (288) if (288) < (XBLOCK) else (XBLOCK)
    x0numel : tl.constexpr = 288
    real_block_x0 : tl.constexpr = (((x_g_tile0) + 7) // 8) * 8
    x0_blocks : tl.constexpr = (x0numel + real_block_x0 - 1) // real_block_x0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpkvt14q2h/qe/cqeyo3wkdryjnzkopgk2pk254a5gkumlhlw4shoz4kss7zqntrsz.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.zeros_like, aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, add_1, convolution, full_default, mul, mul_2, rsqrt, sub, view, view_1, view_2
# Graph fragment:
...
#   %add_1 : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_3, %mul_2), kwargs = {})
#   %convolution : Tensor "f32[4, 32, 47][1504, 47, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.convolution.default](args = (%primals_6, %mul_1, %add_1, [2], [0], [1], False, [0], 1), kwargs = {})
#   return %buf2
triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_2 = async_compile.triton('triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_2', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_2', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 4, 'num_reduction': 0, 'npu_num_x_nodes': 1, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_2(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 32
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpkvt14q2h/4m/c4mg2nn5x26vj7ynlotw2uxrv2ktogq6ftwhfngevkibfgrpbjwe.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.zeros_like, aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, add_1, convolution, full_default, mul, mul_2, rsqrt, sub, view, view_1, view_2
# Graph fragment:
#   %buf3 : Tensor "f32[4, 32, 47][1504, 47, 1]npu:0" = PlaceHolder[target=buf3]
...
#   %convolution : Tensor "f32[4, 32, 47][1504, 47, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.convolution.default](args = (%primals_6, %mul_1, %add_1, [2], [0], [1], False, [0], 1), kwargs = {})
#   return %convolution
triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_3 = async_compile.triton('triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_3', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 6016}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [47, 32, 4, {'divisors': [1, 47, 1504]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 47, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 32, 'divisor': 47, 'seed': 2}, {'name': 'x2', 'length': 4, 'divisor': 1504, 'seed': 2}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_3', 'mutated_arg_names': ['in_out_ptr0'], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 2, 'num_reduction': 0, 'npu_num_x_nodes': 3, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_3(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 6016
    x_g_tile0 : tl.constexpr = (47) if (47) < (XBLOCK) else (XBLOCK)
    x_g_rem1 : tl.constexpr = ((XBLOCK) // x_g_tile0) if ((XBLOCK) // x_g_tile0) > 1 else 1
    x_g_tile1 : tl.constexpr = (32) if (32) < (x_g_rem1) else (x_g_rem1)
    ...
    for i in range(group_size):
        ...
        tmp0 = tl.load(in_out_ptr0 + (x0 + 47*x1 + 1504*x2), x0mask & x1mask & x2mask)
        tmp1 = tl.load(in_ptr0 + (x1), x1mask, eviction_policy='evict_last')
        tmp2 = tmp0 + tmp1
        tl.store(in_out_ptr0 + (x0 + 47*x1 + 1504*x2), tmp2, x0mask & x1mask & x2mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((32, 3, 3), (9, 3, 1), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_view_0.run(primals_4, primals_2, buf0, 288, stream=raw_stream0)
            buf1 = buf0; del buf0  # reuse
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_view_1.run(buf1, primals_5, 288, stream=raw_stream0)
            buf2 = empty_strided_npu((32, ), (1, ), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.zeros_like, aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_2.run(primals_3, primals_4, primals_2, primals_1, buf2, 32, stream=raw_stream0)
            del primals_3
            primals_6 = copy_if_misaligned(primals_6)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.zeros_like, aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
            # [Provenance debug handles] extern_kernels.convolution:1
            buf3 = extern_kernels.convolution(primals_6, buf1, stride=(2,), padding=(0,), dilation=(1,), transposed=False, output_padding=(0,), groups=1, bias=None)
            buf4 = buf3; del buf3  # reuse
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.zeros_like, aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_convolution_mul_rsqrt_sub_view_zeros_like_3.run(buf4, buf2, 6016, stream=raw_stream0)
            del buf2
        return (buf4, primals_1, primals_2, primals_4, primals_5, primals_6, buf1, )
