...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpjqnuxmsm/ym/cymtyega4jjavc2m6lbqdkv4ldkvfjyhibivzoa6yqzc7qsc5bm3.py
...
#   %view : Tensor "f32[1, 32, 1][32, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%rsqrt, [1, -1, 1]), kwargs = {})
...
#   %mul_1 : Tensor "f32[3, 32, 3][96, 3, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_6, %mul), kwargs = {})
#   return %buf0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpjqnuxmsm/l2/cl24nw77vjtesqinpskazmbqcfzs6xyz7ifpicymrnjttmst234x.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, mul, mul_1, rsqrt, view, view_1
# Graph fragment:
#   %primals_6 : Tensor "f32[3, 32, 3][96, 3, 1]npu:0" = PlaceHolder[target=primals_6]
#   %buf0 : Tensor "f32[3, 32, 3][96, 3, 1]npu:0" = PlaceHolder[target=buf0]
#   %add : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_2, 1e-05), kwargs = {})
#   %rsqrt : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add,), kwargs = {})
#   %view : Tensor "f32[1, 32, 1][32, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%rsqrt, [1, -1, 1]), kwargs = {})
#   %view_1 : Tensor "f32[1, 32, 1][32, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%primals_4, [1, 32, 1]), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpjqnuxmsm/6b/c6b6pub3f3afchlateq6wvkruhe6aljh644l2boice2janly2jgw.py
...
#   %convolution : Tensor "f32[4, 32, 9][288, 9, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.convolution.default](args = (%primals_7, %mul_1, %add_1, [2], [0], [1], True, [0], 1), kwargs = {})
#   return %buf2
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpjqnuxmsm/xh/cxhcrtjf4d6ub6syqavwknymtr57iuaf4ao632ijcb3d5wberj6x.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, add_1, convolution, mul, mul_2, rsqrt, sub, view, view_1, view_2
# Graph fragment:
#   %buf3 : Tensor "f32[4, 32, 9][288, 9, 1]npu:0" = PlaceHolder[target=buf3]
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 1152}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [9, 32, 4, {'divisors': [1, 9, 288]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 9, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 32, 'divisor': 9, 'seed': 2}, {'name': 'x2', 'length': 4, 'divisor': 288, 'seed': 2}]},
...
def triton_unk_fused_add_convolution_mul_rsqrt_sub_view_3(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        x2mask = x2index < x2numel
        xmask = x0mask & x1mask & x2mask
        tmp0 = tl.load(in_out_ptr0 + (x0 + 9*x1 + 288*x2), x0mask & x1mask & x2mask)
        tmp1 = tl.load(in_ptr0 + (x1), x1mask, eviction_policy='evict_last')
        tmp2 = tmp0 + tmp1
        tl.store(in_out_ptr0 + (x0 + 9*x1 + 288*x2), tmp2, x0mask & x1mask & x2mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((3, 32, 3), (96, 3, 1), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            ...
            buf3 = extern_kernels.convolution(primals_7, buf1, stride=(2,), padding=(0,), dilation=(1,), transposed=True, output_padding=(0,), groups=1, bias=None)
            buf4 = buf3; del buf3  # reuse
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_convolution_mul_rsqrt_sub_view_3.run(buf4, buf2, 1152, stream=raw_stream0)
            del buf2
