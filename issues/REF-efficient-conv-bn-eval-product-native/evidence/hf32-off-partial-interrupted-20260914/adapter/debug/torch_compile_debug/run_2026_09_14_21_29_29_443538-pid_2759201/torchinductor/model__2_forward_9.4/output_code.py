...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpj6cxzovp/bs/cbsgyg6bupme5ze4uldszptbw2iaehcjpjiioqteu6fuew4gu2w4.py
...
#   %mul_1 : Tensor "f32[32, 3][3, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_6, %mul), kwargs = {})
#   return %buf0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpj6cxzovp/if/cif46rhgrb66ev2uceygexjsisztfofrvrlyrfxjy7pd5vzmvapo.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, mul, mul_1, rsqrt, view, view_1
# Graph fragment:
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmpj6cxzovp/gt/cgtpds7uetctfqiy576evb7s36lqcr3g72ppn5hheifrja6g66im.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, add_1, mul, mul_2, rsqrt, sub, view, view_1, view_2
# Graph fragment:
...
#   return %add_1
triton_unk_fused_add_mul_rsqrt_sub_view_2 = async_compile.triton('triton_unk_fused_add_mul_rsqrt_sub_view_2', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_add_mul_rsqrt_sub_view_2', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 5, 'num_reduction': 0, 'npu_num_x_nodes': 1, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_add_mul_rsqrt_sub_view_2(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 32
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused_add_mul_rsqrt_view_0.run(primals_4, primals_2, buf0, 96, stream=raw_stream0)
            buf1 = buf0; del buf0  # reuse
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_view_1.run(buf1, primals_6, 96, stream=raw_stream0)
            buf2 = empty_strided_npu((32, ), (1, ), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_sub_view_2.run(primals_3, primals_4, primals_2, primals_5, primals_1, buf2, 32, stream=raw_stream0)
            del primals_3
            primals_7 = copy_if_misaligned(primals_7)
            buf3 = empty_strided_npu((4, 32), (32, 1), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.t, aten.addmm]
            # [Provenance debug handles] extern_kernels.addmm:1
            extern_kernels.addmm(buf2, primals_7, reinterpret_tensor(buf1, (3, 32), (1, 3), 0), alpha=1, beta=1, out=buf3)
            del buf1
            del buf2
        return (buf3, primals_1, primals_2, primals_4, primals_5, primals_6, primals_7, )
