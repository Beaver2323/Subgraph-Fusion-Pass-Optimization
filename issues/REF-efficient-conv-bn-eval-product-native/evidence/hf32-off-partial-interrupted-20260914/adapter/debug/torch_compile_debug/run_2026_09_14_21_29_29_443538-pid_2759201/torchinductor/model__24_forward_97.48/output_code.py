...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp00fxni1u/om/comz3pz2m5hf7mcjhjmlxc3ycs6ajws7ijhso5lib56c46borczr.py
...
#   %view : Tensor "f32[1, 32, 1, 1, 1][32, 1, 1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%rsqrt, [1, -1, 1, 1, 1]), kwargs = {})
#   %view_1 : Tensor "f32[1, 32, 1, 1, 1][32, 1, 1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%primals_4, [1, 32, 1, 1, 1]), kwargs = {})
...
#   %mul_1 : Tensor "f32[3, 32, 3, 3, 3][864, 27, 9, 3, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_6, %mul), kwargs = {})
#   return %buf0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp00fxni1u/km/ckm3xd3j3pjx4subdawwuj7omkpen4nrpobeh3qjzq7rc6bmvn2m.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, mul, mul_1, rsqrt, view, view_1
# Graph fragment:
#   %primals_6 : Tensor "f32[3, 32, 3, 3, 3][864, 27, 9, 3, 1]npu:0" = PlaceHolder[target=primals_6]
#   %buf0 : Tensor "f32[3, 32, 3, 3, 3][864, 27, 9, 3, 1]npu:0" = PlaceHolder[target=buf0]
#   %add : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_2, 1e-05), kwargs = {})
#   %rsqrt : Tensor "f32[32][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add,), kwargs = {})
#   %view : Tensor "f32[1, 32, 1, 1, 1][32, 1, 1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%rsqrt, [1, -1, 1, 1, 1]), kwargs = {})
#   %view_1 : Tensor "f32[1, 32, 1, 1, 1][32, 1, 1, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%primals_4, [1, 32, 1, 1, 1]), kwargs = {})
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 2592}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [2592, {'divisors': [1]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 2592, 'divisor': 1, 'seed': 1}]},
...
def triton_unk_fused_add_mul_rsqrt_view_1(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    group_id = tl.program_id(0)
    xnumel = 2592
    x_g_tile0 : tl.constexpr = (2592) if (2592) < (XBLOCK) else (XBLOCK)
    x0numel : tl.constexpr = 2592
    real_block_x0 : tl.constexpr = (((x_g_tile0) + 7) // 8) * 8
    x0_blocks : tl.constexpr = (x0numel + real_block_x0 - 1) // real_block_x0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp00fxni1u/6b/c6b6pub3f3afchlateq6wvkruhe6aljh644l2boice2janly2jgw.py
...
#   %convolution : Tensor "f32[4, 32, 9, 9, 9][23328, 729, 81, 9, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.convolution.default](args = (%primals_7, %mul_1, %add_1, [2, 2, 2], [0, 0, 0], [1, 1, 1], True, [0, 0, 0], 1), kwargs = {})
#   return %buf2
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp00fxni1u/dq/cdq6ol2fzfxx4qyzowl3buhvvr7fwjmybjbvvtl67mbk2uqcvq3h.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, add_1, convolution, mul, mul_2, rsqrt, sub, view, view_1, view_2
# Graph fragment:
#   %buf3 : Tensor "f32[4, 32, 9, 9, 9][23328, 729, 81, 9, 1]npu:0" = PlaceHolder[target=buf3]
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 93312}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [729, 32, 4, {'divisors': [1, 729, 23328]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 729, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 32, 'divisor': 729, 'seed': 2}, {'name': 'x2', 'length': 4, 'divisor': 23328, 'seed': 2}]},
...
def triton_unk_fused_add_convolution_mul_rsqrt_sub_view_3(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    group_id = tl.program_id(0)
    xnumel = 93312
    x_g_tile0 : tl.constexpr = (729) if (729) < (XBLOCK) else (XBLOCK)
    x_g_rem1 : tl.constexpr = ((XBLOCK) // x_g_tile0) if ((XBLOCK) // x_g_tile0) > 1 else 1
    x_g_tile1 : tl.constexpr = (32) if (32) < (x_g_rem1) else (x_g_rem1)
    x_g_rem2 : tl.constexpr = ((x_g_rem1) // x_g_tile1) if ((x_g_rem1) // x_g_tile1) > 1 else 1
    x_g_tile2 : tl.constexpr = (4) if (4) < (x_g_rem2) else (x_g_rem2)
    x0numel : tl.constexpr = 729
    x1numel : tl.constexpr = 32
    ...
    for i in range(group_size):
        ...
        x2mask = x2index < x2numel
        xmask = x0mask & x1mask & x2mask
        tmp0 = tl.load(in_out_ptr0 + (x0 + 729*x1 + 23328*x2), x0mask & x1mask & x2mask)
        tmp1 = tl.load(in_ptr0 + (x1), x1mask, eviction_policy='evict_last')
        tmp2 = tmp0 + tmp1
        tl.store(in_out_ptr0 + (x0 + 729*x1 + 23328*x2), tmp2, x0mask & x1mask & x2mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((3, 32, 3, 3, 3), (864, 27, 9, 3, 1), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_view_0.run(primals_4, primals_2, buf0, 2592, stream=raw_stream0)
            buf1 = buf0; del buf0  # reuse
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_view_1.run(buf1, primals_6, 2592, stream=raw_stream0)
            ...
            buf3 = extern_kernels.convolution(primals_7, buf1, stride=(2, 2, 2), padding=(0, 0, 0), dilation=(1, 1, 1), transposed=True, output_padding=(0, 0, 0), groups=1, bias=None)
            buf4 = buf3; del buf3  # reuse
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub, aten.convolution]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_convolution_mul_rsqrt_sub_view_3.run(buf4, buf2, 93312, stream=raw_stream0)
            del buf2
...
def get_args():
    ...
    primals_6 = rand_strided((3, 32, 3, 3, 3), (864, 27, 9, 3, 1), device='npu:0', dtype=torch.float32)
    primals_7 = rand_strided((4, 3, 4, 4, 4), (192, 64, 16, 4, 1), device='npu:0', dtype=torch.float32)
