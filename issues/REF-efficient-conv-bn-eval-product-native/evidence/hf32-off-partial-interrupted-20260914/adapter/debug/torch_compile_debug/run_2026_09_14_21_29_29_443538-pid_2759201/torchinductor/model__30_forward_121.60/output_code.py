...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp685g29ox/bs/cbsgyg6bupme5ze4uldszptbw2iaehcjpjiioqteu6fuew4gu2w4.py
...
#   %mul_1 : Tensor "f32[32, 3][3, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_5, %mul), kwargs = {})
#   return %buf0
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp685g29ox/if/cif46rhgrb66ev2uceygexjsisztfofrvrlyrfxjy7pd5vzmvapo.py
# Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
# Source node to ATen node mapping:
#   efficient_conv_bn_eval => add, mul, mul_1, rsqrt, view, view_1
# Graph fragment:
#   %primals_5 : Tensor "f32[32, 3][3, 1]npu:0" = PlaceHolder[target=primals_5]
...
# kernel path: /home/z50063656/tmp/t098-native-doq0hsal/adapter/inductor-cache/tmp685g29ox/un/cunjz7ogfjm476oq7ct3fyqasd4s2663htkfbxqgt56asqy2tk4b.py
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.add, aten.rsqrt, aten.view, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_view_1.run(buf1, primals_5, 96, stream=raw_stream0)
            buf2 = empty_strided_npu((32, ), (1, ), torch.float32)
            # Topologically Sorted Source Nodes: [efficient_conv_bn_eval], Original ATen: [aten.zeros_like, aten.add, aten.rsqrt, aten.view, aten.mul, aten.sub]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_mul_rsqrt_sub_view_zeros_like_2.run(primals_3, primals_4, primals_2, primals_1, buf2, 32, stream=raw_stream0)
            del primals_3
            ...
            extern_kernels.addmm(buf2, primals_6, reinterpret_tensor(buf1, (3, 32), (1, 3), 0), alpha=1, beta=1, out=buf3)
            del buf1
            del buf2
        return (buf3, primals_1, primals_2, primals_4, primals_5, primals_6, )
...
def get_args():
    ...
    primals_5 = rand_strided((32, 3), (3, 1), device='npu:0', dtype=torch.float32)
    primals_6 = rand_strided((4, 3), (3, 1), device='npu:0', dtype=torch.float32)
