...
# kernel path: /home/z50063656/tmp/t102-training-boundary-candidate-20260915/inductor-cache/tmpite43gby/ek/cekgm5dvp6ps52i6cpclrek3mluhqmalg63auxf4v5lm6eipezmd.py
...
# Graph fragment:
#   %getitem_4 : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0" = PlaceHolder[target=getitem_4]
#   %where_self : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0" = PlaceHolder[target=where_self]
#   %sum_dim_int_list_1 : Tensor "f32[2, 2, 8, 1][16, 8, 1, 32]npu:0" = PlaceHolder[target=sum_dim_int_list_1]
#   %mul_tensor : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%getitem_4, %where_self), kwargs = {})
#   %sum_dim_int_list_1 : Tensor "f32[2, 2, 8, 1][16, 8, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%mul_tensor, [-1], True), kwargs = {})
#   %mul_tensor_1 : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%where_self, %sum_dim_int_list_1), kwargs = {})
#   %sub_tensor_1 : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%mul_tensor, %mul_tensor_1), kwargs = {})
...
def triton_unk_fused__softmax_backward_data_matmul_backward_0(in_out_ptr0, in_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp9 = tmp7 * tmp4
            tmp10 = tmp8 - tmp9
            tl.store(in_out_ptr0 + (r0_1 + 8*x0), tmp10, r0_mask & x0mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t102-training-boundary-candidate-20260915/inductor-cache/tmpite43gby/if/cif3qnjz64fomyyx4ojqnreqiwcko2joy5apu3esloou345txiea.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten._softmax_backward_data, aten.matmul_backward]
# Source node to ATen node mapping:
#    => matmul_backward_default_1, mul_tensor, mul_tensor_1, sub_tensor_1
# Graph fragment:
#   %mul_scalar_1 : Tensor "f32[2, 2, 16, 8][256, 128, 1, 16]npu:0" = PlaceHolder[target=mul_scalar_1]
#   %mul_tensor : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%getitem_4, %where_self), kwargs = {})
#   %mul_tensor_1 : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%where_self, %sum_dim_int_list_1), kwargs = {})
#   %sub_tensor_1 : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%mul_tensor, %mul_tensor_1), kwargs = {})
#   %matmul_backward_default_1 : [num_users=2] = call_function[target=torch.ops.aten.matmul_backward.default](args = (%sub_tensor_1, %mul_scalar, %mul_scalar_1, [True, True]), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t102-training-boundary-candidate-20260915/inductor-cache/tmpite43gby/o4/co4pipumvyqxtod6wvmu5edverhdwi6y7bsx65y3iottbb7umhed.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.mul]
# Source node to ATen node mapping:
#    => mul_scalar_2
# Graph fragment:
#   %getitem_7 : Tensor "f32[2, 2, 16, 8][256, 128, 8, 1]npu:0" = PlaceHolder[target=getitem_7]
#   %mul_scalar_2 : Tensor "f32[2, 2, 16, 8][256, 128, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Scalar](args = (%getitem_7, 0.5), kwargs = {})
#   return %mul_scalar_2
...
class Runner:
    ...
    def call(self, args):
        primals_4, mul_scalar, mul_scalar_1, where_self, tangents_1 = args
        args.clear()
        with torch.npu.utils.device(0):
            ...
            buf0 = torch.ops.aten.matmul_backward.default(tangents_1, where_self, primals_4, [True, True])
            del primals_4
            del tangents_1
            buf1 = buf0[0]
            ...
            triton_unk_fused__softmax_backward_data_matmul_backward_0.run(buf4, where_self, 32, 8, stream=raw_stream0)
            del where_self
            buf5 = empty_strided_npu((2, 2, 16, 8), (256, 128, 8, 1), torch.float32)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten._softmax_backward_data, aten.matmul_backward]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_backward_data_matmul_backward_1.run(mul_scalar_1, buf5, 64, 8, stream=raw_stream0)
            del mul_scalar_1
            # Topologically Sorted Source Nodes: [], Original ATen: [aten._softmax_backward_data, aten.matmul_backward]
            # [Provenance debug handles] torch.ops.aten.matmul_backward.default:8
            ...
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_mul_2.run(buf9, 512, stream=raw_stream0)
            buf10 = buf7; del buf7  # reuse
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul_backward]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_mul_2.run(buf10, 512, stream=raw_stream0)
        return (reinterpret_tensor(buf9, (2, 2, 8, 16), (256, 128, 1, 8), 0), buf10, None, buf2, )
...
def get_args():
    ...
    mul_scalar = rand_strided((2, 2, 8, 16), (256, 128, 16, 1), device='npu:0', dtype=torch.float32)
    mul_scalar_1 = rand_strided((2, 2, 16, 8), (256, 128, 1, 16), device='npu:0', dtype=torch.float32)
    where_self = rand_strided((2, 2, 8, 8), (128, 64, 8, 1), device='npu:0', dtype=torch.float32)
    tangents_1 = rand_strided((2, 2, 8, 16), (256, 128, 16, 1), device='npu:0', dtype=torch.float32)
    return [primals_4, mul_scalar, mul_scalar_1, where_self, tangents_1]
