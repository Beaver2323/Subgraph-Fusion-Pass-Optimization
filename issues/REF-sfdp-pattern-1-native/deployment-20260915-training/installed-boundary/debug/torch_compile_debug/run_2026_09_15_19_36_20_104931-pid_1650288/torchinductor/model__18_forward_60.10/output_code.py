...
# kernel path: /home/z50063656/tmp/t102-training-boundary-installed-20260915/inductor-cache/tmphooftmq6/mm/cmmqsg37kbbjjf2d2srd5363y4smhcie73amlibh4w5udr6rf7jj.py
# Topologically Sorted Source Nodes: [matmul, div, add, softmax], Original ATen: [aten.matmul, aten.div, aten.add, aten._softmax]
...
#   %amax : Tensor "f32[2, 2, 8, 1][16, 8, 1, 32]npu:0" = PlaceHolder[target=amax]
#   %sum_1 : Tensor "f32[2, 2, 8, 1][16, 8, 1, 32]npu:0" = PlaceHolder[target=sum_1]
#   %view_2 : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%bmm, [2, 2, 8, 8]), kwargs = {})
#   %div : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%view_2, 4.0), kwargs = {})
#   %add : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.add.Tensor](args = (%div, %primals_3), kwargs = {})
#   %amax : Tensor "f32[2, 2, 8, 1][16, 8, 1, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.amax.default](args = (%add, [-1], True), kwargs = {})
#   %sub : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%add, %amax), kwargs = {})
#   %exp : Tensor "f32[2, 2, 8, 8][128, 64, 8, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.exp.default](args = (%sub,), kwargs = {})
...
def triton_unk_fused__softmax_add_div_matmul_0(in_out_ptr0, in_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        tmp4 = tmp3
        _tmp7 = tl.full([real_block_x0, R0_BLOCK], float("-inf"), tl.float32)
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp1 = tl.full([1, 1], 0.25, tl.float32)
            tmp2 = tmp0 * tmp1
            tmp5 = tmp2 + tmp4
            tmp6 = tl.broadcast_to(tmp5, [real_block_x0, R0_BLOCK])
            tmp8 = tl.maximum(_tmp7, tmp6)
            _tmp7 = tmp8
        tmp7 = triton_helpers.max2(_tmp7, 1)[:, None]
        tmp12 = tl.load(in_ptr0 + (0))
        tmp13 = tl.broadcast_to(tmp12, [1])
        _tmp18 = tl.full([real_block_x0, R0_BLOCK], 0, tl.float32)
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            r0_index = r0_offset + r0_base
            r0_mask = r0_index < r0_numel
            roffset = r0_offset
            rindex = r0_index
            r0_1 = r0_index
            tmp9 = tl.load(in_out_ptr0 + (r0_1 + 8*x0), r0_mask & x0mask, eviction_policy='evict_last', other=float("-inf"))
            tmp10 = tl.full([1, 1], 0.25, tl.float32)
            tmp11 = tmp9 * tmp10
            tmp14 = tmp11 + tmp13
            tmp15 = tmp14 - tmp7
            tmp16 = libdevice.exp(tmp15)
            tmp17 = tl.broadcast_to(tmp16, [real_block_x0, R0_BLOCK])
            tmp19 = _tmp18 + tmp17
            _tmp18 = tmp19
        tmp18 = tl.sum(_tmp18, 1)[:, None]
        tmp23 = tl.load(in_ptr0 + (0))
        tmp24 = tl.broadcast_to(tmp23, [1])
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            r0_index = r0_offset + r0_base
            r0_mask = r0_index < r0_numel
            roffset = r0_offset
            rindex = r0_index
            r0_1 = r0_index
            tmp20 = tl.load(in_out_ptr0 + (r0_1 + 8*x0), r0_mask & x0mask, eviction_policy='evict_first', other=0.0)
            tmp21 = tl.full([1, 1], 0.25, tl.float32)
            tmp22 = tmp20 * tmp21
            tmp25 = tmp22 + tmp24
            tmp26 = tmp25 - tmp7
            tmp27 = libdevice.exp(tmp26)
            tmp28 = (tmp27 / tmp18)
            tl.store(in_out_ptr0 + (r0_1 + 8*x0), tmp28, r0_mask & x0mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf3 = reinterpret_tensor(buf0, (2, 2, 8, 8), (128, 64, 8, 1), 0); del buf0  # reuse
            # Topologically Sorted Source Nodes: [matmul, div, add, softmax], Original ATen: [aten.matmul, aten.div, aten.add, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax_add_div_matmul_0.run(buf3, primals_3, 32, 8, stream=raw_stream0)
            del primals_3
            primals_4 = copy_if_misaligned(primals_4)
            buf4 = empty_strided_npu((4, 8, 16), (128, 16, 1), torch.float32)
            # Topologically Sorted Source Nodes: [matmul_1], Original ATen: [aten.matmul]
            # [Provenance debug handles] extern_kernels.bmm:2
            extern_kernels.bmm(reinterpret_tensor(buf3, (4, 8, 8), (64, 8, 1), 0), reinterpret_tensor(primals_4, (4, 8, 16), (128, 16, 1), 0), out=buf4)
        return (reinterpret_tensor(buf4, (2, 2, 8, 16), (256, 128, 16, 1), 0), primals_2, primals_4, reinterpret_tensor(primals_1, (2, 2, 16, 8), (256, 128, 1, 16), 0), buf3, )
