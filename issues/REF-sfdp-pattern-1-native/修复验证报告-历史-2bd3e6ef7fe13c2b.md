...
test_sdpa_rewriter_1_gpu → _check_common → torch.compile(fullgraph=True)
...
| NPU 修复前 | 原训练合同 FAIL | [原 result](evidence/t102-native-oen9jamj/adapter/result.json) |
| 隔离共同候选 | 原方法 PASS，不算安装态 | [候选 result](evidence/t102-native-m971rqag/adapter/result.json) |
| 安装态无候选新进程 | tests=1、skip=0、本编号改写 8 次 | [新 result](evidence/t102-native-9ebqlhu3/adapter/result.json) |
| 部署前/后设备边界 | 各 6 场景、25 次输出/梯度比较；负例不命中 | [全部边界](../REF-sfdp-pattern-1-native/deployment-20260915-training/) |

原方法 20 次输出/梯度比较通过，dtype 范围：torch.float16, torch.float32。

按本编号实际改写计数，不以总 fuse_attention counter 替代精确归因。
...
/home/z50063656/envs/Pass/bin/python /home/z50063656/Pass/Subgraph-Fusion-Pass-Optimization/issues/REF-sfdp-pattern-1-native/npu_adapter.py --artifact-dir /home/z50063656/tmp/t102-native-9ebqlhu3/adapter
```

[完整父命令/哈希/退出码及 stdout/stderr](adapter_runs/native-20260915T191925-k74ih7rf/)。返回码 0，`Ran 1 test` / `OK`，不含候选开关。
...
|---|---:|---:|---:|
| host_ms p50 | 0.750280 | 0.540915 | +27.90% |
| host_ms p99 | 0.809014 | 0.623364 | +22.95% |
| event_ms p50 | 0.633330 | 0.415580 | +34.38% |
| event_ms p99 | 0.672853 | 0.469827 | +30.17% |

同 NPU 5、六个新进程，顺序 OFF1/ON1/ON2/OFF2/OFF3/ON3；每臂预热10次、host及Event各100样本。汇总取三轮各臂p50/p99的中位数，原样本、波动和compile_ms保留。compile_ms起点位于先行lazy_init之后，不是包含训练注册trace的完整冷启动成本。[功能门禁](../../results/current/T-102/performance_gates/pattern-1-executed.json)、[完整验签包](../../results/current/T-102/attention_bundles/pattern-1.json)、[时延原始记录](../../results/current/T-102/performance_summary.json)。
