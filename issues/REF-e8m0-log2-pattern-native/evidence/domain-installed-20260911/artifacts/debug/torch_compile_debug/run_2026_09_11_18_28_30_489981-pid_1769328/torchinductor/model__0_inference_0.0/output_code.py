...
# kernel path: /home/z50063656/tmp/t096-domain-installed-3Rqrwy/artifacts/inductor-cache/tmpdnpon3bs/j6/cj63dqgpeeshkldsyfsc4ogpxrnnjikz6ascbgrrbb6dmolyfyeo.py
...
# Graph fragment:
#   %view_dtype : Tensor "i32[22][1]npu:0" = PlaceHolder[target=view_dtype]
#   %__rshift___scalar : Tensor "i32[22][1]npu:0" = PlaceHolder[target=__rshift___scalar]
#   %bitwise_and_scalar : Tensor "i32[22][1]npu:0"[num_users=3] = call_function[target=torch.ops.aten.bitwise_and.Scalar](args = (%__rshift___scalar, 255), kwargs = {})
#   %bitwise_and_scalar_1 : Tensor "i32[22][1]npu:0"[num_users=3] = call_function[target=torch.ops.aten.bitwise_and.Scalar](args = (%view_dtype, 8388607), kwargs = {})
#   %ne_scalar : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.ne.Scalar](args = (%bitwise_and_scalar_1, 0), kwargs = {})
#   %convert_element_type_default : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%ne_scalar, torch.int32), kwargs = {})
#   %add_tensor : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%bitwise_and_scalar, %convert_element_type_default), kwargs = {})
#   %clamp_min_default : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.clamp_min.default](args = (%add_tensor, 0), kwargs = {})
#   %clamp_max_default : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.clamp_max.default](args = (%clamp_min_default, 254), kwargs = {})
#   %gt_scalar : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.gt.Scalar](args = (%bitwise_and_scalar_1, 4194304), kwargs = {})
#   %convert_element_type_default_1 : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%gt_scalar, torch.int32), kwargs = {})
#   %eq_scalar : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%bitwise_and_scalar, 0), kwargs = {})
#   %where_self : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%eq_scalar, %convert_element_type_default_1, %clamp_max_default), kwargs = {})
#   %lt_scalar : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.lt.Scalar](args = (%view_dtype, 0), kwargs = {})
#   %eq_scalar_1 : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%bitwise_and_scalar, 255), kwargs = {})
#   %ne_scalar_1 : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.ne.Scalar](args = (%bitwise_and_scalar_1, 0), kwargs = {})
#   %bitwise_and_tensor : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.bitwise_and.Tensor](args = (%eq_scalar_1, %ne_scalar_1), kwargs = {})
#   %bitwise_or_tensor : Tensor "b8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.bitwise_or.Tensor](args = (%lt_scalar, %bitwise_and_tensor), kwargs = {})
...
#   %where_self_1 : Tensor "i32[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%bitwise_or_tensor, %scalar_tensor_default, %where_self), kwargs = {})
#   %convert_element_type_default_2 : Tensor "u8[22][1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%where_self_1, torch.uint8), kwargs = {})
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 22}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i32', 'in_ptr1': '*i32', 'out_ptr0': '*u8', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [22, {'divisors': [1]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 22, 'divisor': 1, 'seed': 1}]},
...
def triton_unk_fused__to_copy_add_bitwise_and_bitwise_or_clamp_eq_gt_lt_ne_scalar_tensor_where_0(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    xnumel = 22
    x_g_tile0 : tl.constexpr = (22) if (22) < (XBLOCK) else (XBLOCK)
    x0numel : tl.constexpr = 22
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf2 = empty_strided_npu((22, ), (1, ), torch.uint8)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.bitwise_and, aten.ne, aten._to_copy, aten.add, aten.clamp, aten.gt, aten.eq, aten.where, aten.lt, aten.bitwise_or, aten.scalar_tensor]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__to_copy_add_bitwise_and_bitwise_or_clamp_eq_gt_lt_ne_scalar_tensor_where_0.run(buf0, buf1, buf2, 22, stream=raw_stream0)
            del arg0_1
            del buf0
            del buf1
        return (buf2, )
...
def get_args():
    ...
    arg0_1 = rand_strided((22, ), (1, ), device='npu:0', dtype=torch.float32)
    return [arg0_1]
