#!/usr/bin/env python3
"""static-fp16 pad-bmm community case 的 NPU 产品基线适配器。"""
...
def load_upstream(root: Path):
    ...
    spec = importlib.util.spec_from_file_location("t076_pad_bmm_static", source)
...
def main() -> int:
    ...
    class Case(unittest.TestCase):
        ...
        def test_product_baseline(self):
            inputs = (
                torch.ones((3, 6, 11), device="npu", dtype=torch.float16),
                torch.ones((3, 11, 9), device="npu", dtype=torch.float16),
            )
            expected_alignment = upstream.get_alignment_size(inputs[0])
            expected = torch.bmm(*inputs)
            ...
            marker = "empty_strided_npu((3, 8, 16)"
            self.assertEqual(expected_alignment, 8)
    ...
    summary = {
        "source_test": "test/inductor/test_pad_mm.py::PadMMTest.test_pad_batch",
        "adapter_deviation": ["GPU_TYPE=npu", "backend=triton_experimental", "bypass GPU-only big-GPU harness", "allow ATEN choice", "map padded allocation positive to product-gate negative"],
