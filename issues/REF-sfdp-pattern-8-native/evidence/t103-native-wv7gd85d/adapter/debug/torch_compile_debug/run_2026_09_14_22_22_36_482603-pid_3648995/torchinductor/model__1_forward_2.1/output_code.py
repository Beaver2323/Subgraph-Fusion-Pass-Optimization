...
# kernel path: /home/z50063656/tmp/t103-native-wv7gd85d/adapter/inductor-cache/tmp1kawwdcb/sy/csy6kabl7aduvd3axetllews4kuja4qp6tory7yfgyhdl4xxhysd.py
...
# kernel path: /home/z50063656/tmp/t103-native-wv7gd85d/adapter/inductor-cache/tmp1kawwdcb/ue/cuegvse4q6qaiknsrinclsedyhzkb3f7dj6bqqlqcs6hazcpgm7l.py
...
# kernel path: /home/z50063656/tmp/t103-native-wv7gd85d/adapter/inductor-cache/tmp1kawwdcb/so/csosfx2pljscnemrz7qzfpshg2pivciphbuy6w4mvtpo5iosy3s3.py
...
# kernel path: /home/z50063656/tmp/t103-native-wv7gd85d/adapter/inductor-cache/tmp1kawwdcb/x5/cx55fmg23cnudfkicmk3khqv4ue5mgzid2llit4ehvbgp6xxn7pz.py
# Topologically Sorted Source Nodes: [matmul, , softmax, to_1], Original ATen: [aten.matmul, aten.div, aten._to_copy, aten.mul, aten.amax, aten.sub, aten.all, aten._softmax]
# Source node to ATen node mapping:
...
#   %div_1 : Tensor "f32[2, 4, 8, 8][256, 64, 8, 1]npu:0" = PlaceHolder[target=div_1]
#   %view_2 : Tensor "f16[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.reshape.default](args = (%bmm, [2, 4, 8, 8]), kwargs = {})
#   %div_tensor : Tensor "f16[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%view_2, 4.0), kwargs = {})
#   %convert_element_type_default : Tensor "f32[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=4] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%div_tensor, torch.float32), kwargs = {})
...
#   return %amax_default,%amax_default_1,%sum_1,%div_1,%expand_2
...
@triton.jit
def triton_unk_fused__softmax__to_copy_all_amax_div_matmul_mul_sub_3(in_ptr0, in_ptr1, out_ptr3, out_ptr4, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 64
    r0_numel = 8
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf9 = empty_strided_npu((2, 4, 8, 8), (256, 64, 8, 1), torch.float16)
            # Topologically Sorted Source Nodes: [matmul, , softmax, to_1], Original ATen: [aten.matmul, aten.div, aten._to_copy, aten.mul, aten.amax, aten.sub, aten.all, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_all_amax_div_matmul_mul_sub_3.run(buf2, buf6, buf8, buf9, 64, 8, stream=raw_stream0)
            del buf2
            del buf6
            ...
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_matmul_permute_0.run(primals_3, buf10, 1024, stream=raw_stream0)
            ...
            del buf10
        return (reinterpret_tensor(buf11, (2, 4, 8, 16), (512, 128, 16, 1), 0), reinterpret_tensor(primals_2, (2, 4, 8, 16), (512, 16, 64, 1), 0), reinterpret_tensor(primals_3, (2, 4, 8, 16), (512, 16, 64, 1), 0), reinterpret_tensor(primals_1, (2, 4, 16, 8), (512, 16, 1, 64), 0), buf8, buf9, )
