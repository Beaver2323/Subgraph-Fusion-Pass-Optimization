...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232256+0800/pattern-18/on/inductor-cache/tmp5zc6l3l3/7j/c7j5rm7mpnrwuikzvw44nfhkku5qmcnhhmufht2iz3i7kzuxz7k6.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.transpose, aten.permute, aten.expand, aten.logical_not, npu.npu_fusion_attention_v3]
# Source node to ATen node mapping:
#    => expand_default, logical_not_default, npu_fusion_attention_v3_default, permute_default, permute_default_1, permute_default_2
# Graph fragment:
#   %arg3_1 : Tensor "b8[2, 1, 1, 4][4, 4, 4, 1]npu:0" = PlaceHolder[target=arg3_1]
#   %permute_default_2 : Tensor "f16[2, 8, 4, 16][512, 16, 128, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%arg0_1, [0, 2, 1, 3]), kwargs = {})
#   %permute_default : Tensor "f16[2, 8, 4, 16][512, 16, 128, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%arg1_1, [0, 2, 1, 3]), kwargs = {})
#   %permute_default_1 : Tensor "f16[2, 8, 4, 16][512, 16, 128, 1]npu:0"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%arg2_1, [0, 2, 1, 3]), kwargs = {})
#   %expand_default : Tensor "b8[2, 8, 4, 4][4, 0, 0, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%arg3_1, [2, 8, 4, 4]), kwargs = {})
#   %logical_not_default : Tensor "b8[2, 8, 4, 4][128, 16, 4, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.logical_not.default](args = (%expand_default,), kwargs = {})
#   %npu_fusion_attention_v3_default : [num_users=1] = call_function[target=torch.ops.npu.npu_fusion_attention_v3.default](args = (%permute_default_2, %permute_default, %permute_default_1, 8, BNSD, None, None, %logical_not_default, 1.5000150001500014, 1.0, 2147483647, 2147483647, 2), kwargs = {})
#   return %buf0
triton_unk_fused_expand_logical_not_npu_fusion_attention_v3_permute_transpose_0 = async_compile.triton('triton_unk_fused_expand_logical_not_npu_fusion_attention_v3_permute_transpose_0', '''
import triton
import triton.language as tl
...
def triton_unk_fused_expand_logical_not_npu_fusion_attention_v3_permute_transpose_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        tmp1 = tmp0 == 0
        tl.store(out_ptr0 + (x0 + 4*x1 + 128*x2), tmp1, x0mask & x1mask & x2mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((2, 8, 4, 4), (128, 16, 4, 1), torch.bool)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.transpose, aten.permute, aten.expand, aten.logical_not, npu.npu_fusion_attention_v3]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_expand_logical_not_npu_fusion_attention_v3_permute_transpose_0.run(arg3_1, buf0, 256, stream=raw_stream0)
            del arg3_1
            ...
            buf1 = torch.ops.npu.npu_fusion_attention_v3.default(reinterpret_tensor(arg0_1, (2, 8, 4, 16), (512, 16, 128, 1), 0), reinterpret_tensor(arg1_1, (2, 8, 4, 16), (512, 16, 128, 1), 0), reinterpret_tensor(arg2_1, (2, 8, 4, 16), (512, 16, 128, 1), 0), 8, 'BNSD', None, None, buf0, 1.5000150001500014, 1.0, 2147483647, 2147483647, 2, prefix=None, actual_seq_qlen=None, actual_seq_kvlen=None, sparse_mode=0, gen_mask_parallel=True, sync=False, softmax_layout='', sink=None)
            del arg0_1
            del buf0
            buf2 = buf1[0]
            assert_alignment(buf2, 16, 'torch.ops.npu.npu_fusion_attention_v3.default')
            del buf1
        return (buf2, reinterpret_tensor(arg1_1, (2, 8, 4, 16), (512, 16, 128, 1), 0), reinterpret_tensor(arg2_1, (2, 8, 4, 16), (512, 16, 128, 1), 0), )
