# [Fix] Align experimental SFDP pattern 2 training registration with NPU decomposition
...
同一候选五个原方法、安装态五个原方法、部署前后各六项设备边界通过。原方法 10 次输出/梯度比较通过，dtype 范围：torch.float32。 按本编号实际改写计数，不以总 fuse_attention counter 替代精确归因。
