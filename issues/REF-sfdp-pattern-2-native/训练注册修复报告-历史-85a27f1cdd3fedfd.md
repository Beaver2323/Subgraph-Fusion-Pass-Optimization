...
test_sdpa_rewriter_2_gpu → _check_common → torch.compile(fullgraph=True)
...
| NPU 修复前 | 原训练合同 FAIL | [原 result](evidence/t102-native-iqguocqa/adapter/result.json) |
| 隔离共同候选 | 原方法 PASS，不算安装态 | [候选 result](evidence/t102-native-ibuudjyk/adapter/result.json) |
| 安装态无候选新进程 | tests=1、skip=0、本编号改写 4 次 | [新 result](evidence/t102-native-xfsnbo7j/adapter/result.json) |
| 部署前/后设备边界 | 各 6 场景、25 次输出/梯度比较；负例不命中 | [全部边界](../REF-sfdp-pattern-1-native/deployment-20260915-training/) |

原方法 10 次输出/梯度比较通过，dtype 范围：torch.float32。
...
/home/z50063656/envs/Pass/bin/python /home/z50063656/Pass/Subgraph-Fusion-Pass-Optimization/issues/REF-sfdp-pattern-2-native/npu_adapter.py --artifact-dir /home/z50063656/tmp/t102-native-xfsnbo7j/adapter
```

[完整父命令/哈希/退出码及 stdout/stderr](adapter_runs/native-20260915T192005-x2cxinle/)。返回码 0，`Ran 1 test` / `OK`，不含候选开关。
