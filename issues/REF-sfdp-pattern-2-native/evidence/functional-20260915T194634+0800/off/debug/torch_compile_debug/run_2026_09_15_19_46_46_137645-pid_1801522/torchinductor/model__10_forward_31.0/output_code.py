...
# kernel path: /home/z50063656/tmp/t102-training-performance-20260915/functional-20260915T194634+0800/pattern-2/off/inductor-cache/tmpwk57g_7g/pm/cpm4bzsrn52dbhwxuzla5vykjc7f5qtrvm5ivmd7624fpdfrlc2g.py
...
#   %mul_tensor : Tensor "f16[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view_2, 2.0), kwargs = {})
...
@triton.jit
def triton_unk_fused__to_copy_all_isfinite_matmul_mul_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
...
# kernel path: /home/z50063656/tmp/t102-training-performance-20260915/functional-20260915T194634+0800/pattern-2/off/inductor-cache/tmpwk57g_7g/rl/crl4ukrxcs47cutdhlcgo5s5ratsu6hdyxahzzans7fmyjzi2ph6.py
...
#   %mul_tensor_2 : Tensor "f32[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sub_tensor, 2.0), kwargs = {})
...
triton_unk_fused__softmax__to_copy_all_amax_matmul_mul_sub_1 = async_compile.triton('triton_unk_fused__softmax__to_copy_all_amax_matmul_mul_sub_1', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.reduction(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__softmax__to_copy_all_amax_matmul_mul_sub_1', 'mutated_arg_names': ['in_out_ptr0'], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 4, 'num_reduction': 3, 'npu_num_x_nodes': 1, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False}
)
@triton.jit
def triton_unk_fused__softmax__to_copy_all_amax_matmul_mul_sub_1(in_out_ptr0, in_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 64
    r0_numel = 8
    ...
    for i in range(group_size):
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp18 = tmp16 * tmp17
            tmp19 = tmp18 - tmp5
            tmp20 = tl.full([1, 1], 2.0, tl.float32)
            tmp21 = tmp19 * tmp20
            tmp22 = tmp15 * tmp20
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp34 = tmp32 * tmp33
            tmp35 = tmp34 - tmp5
            tmp36 = tl.full([1, 1], 2.0, tl.float32)
            tmp37 = tmp35 * tmp36
            tmp38 = tmp31 * tmp36
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused__to_copy_all_isfinite_matmul_mul_0.run(buf0, buf3, 512, stream=raw_stream0)
            # Topologically Sorted Source Nodes: [matmul, ], Original ATen: [aten.matmul, aten.mul, aten._to_copy, aten.isfinite, aten.all]
            ...
            # Topologically Sorted Source Nodes: [matmul, , softmax], Original ATen: [aten.matmul, aten.mul, aten._to_copy, aten.amax, aten.sub, aten.all, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_all_amax_matmul_mul_sub_1.run(buf6, buf4, 64, 8, stream=raw_stream0)
            del buf4
            primals_3 = copy_if_misaligned(primals_3)
            buf7 = empty_strided_npu((8, 8, 16), (128, 16, 1), torch.float16)
        ...
        return (reinterpret_tensor(buf7, (2, 4, 8, 16), (512, 128, 16, 1), 0), primals_2, primals_3, reinterpret_tensor(primals_1, (2, 4, 16, 8), (512, 128, 1, 16), 0), buf6, )
