...
# kernel path: /home/z50063656/tmp/t105-native-vnk4yrre/adapter/inductor-cache/tmp4my8qy_g/pl/cpl2uyu3futnfioiuu6csryhff3kwdkq6sjmlhpzxr3pjf4nqhdt.py
...
#   %mul_scalar : Tensor "f32[4, 2, 16, 32][1024, 512, 32, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Scalar](args = (%arg3_1, 0.42044820762685725), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t105-native-vnk4yrre/adapter/inductor-cache/tmp4my8qy_g/a4/ca4iqk4i44czmmam2alwfiv7ivb6kfykz24mvb7t63ty2m7q3s5m.py
...
#   %permute_default : Tensor "f32[4, 2, 32, 16][1024, 512, 1, 32]npu:0"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%arg2_1, [0, 1, 3, 2]), kwargs = {})
#   %mul_scalar_1 : Tensor "f32[4, 2, 32, 16][1024, 512, 1, 32]npu:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Scalar](args = (%permute_default, 0.42044820762685725), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t105-native-vnk4yrre/adapter/inductor-cache/tmp4my8qy_g/y3/cy3tr7pb3xu4cwr52bxhlqh2gsffcxqxl6mgoo5xdfpoj576oh4e.py
...
#   %arg1_1 : Tensor "b8[16, 16][16, 1]npu:0" = PlaceHolder[target=arg1_1]
#   %arg0_1 : Tensor "f32[16, 16][16, 1]npu:0" = PlaceHolder[target=arg0_1]
...
#   %where_self : Tensor "f32[16, 16][16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%arg1_1, %arg0_1, %full_default_2), kwargs = {})
#   %add_tensor : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=3] = call_function[target=torch.ops.aten.add.Tensor](args = (%view_default_2, %where_self), kwargs = {})
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 2048}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*i1', 'in_ptr2': '*fp32', 'out_ptr0': '*i1', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2, 3, 4), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [256, 8, {'divisors': [1, 256]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 256, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 8, 'divisor': 256, 'seed': 2}]},
...
def triton_unk_fused__safe_softmax_add_full_matmul_where_2(in_ptr0, in_ptr1, in_ptr2, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        tmp6 = tmp5 == tmp3
        tmp7 = tmp6 == 0
        tl.store(out_ptr0 + (x0 + 256*x1), tmp7, x0mask & x1mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t105-native-vnk4yrre/adapter/inductor-cache/tmp4my8qy_g/uj/cujtcpi2gjw7p5qvrm765b5bxklrfulkmynui3urvp7hzb46cqfz.py
# Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.full, aten.where, aten.add, aten._safe_softmax]
# Source node to ATen node mapping:
...
#   %full_default_3 : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([4, 2, 16, 16], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: npu:0, pin_memory: False})
...
#   %where_self_1 : Tensor "f32[4, 2, 16, 16][512, 256, 16, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%logical_not_default_1, %full_default_3, %div_tensor), kwargs = {})
...
def triton_unk_fused__safe_softmax_add_full_matmul_where_3(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            rindex = r0_index
            r0_2 = r0_index
            tmp0 = tl.load(in_out_ptr0 + (r0_2 + 16*x0 + 256*x1), r0_mask & x0mask & x1mask, eviction_policy='evict_last', other=0.0)
            tmp1 = tl.load(in_ptr0 + (r0_2 + 16 * x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0) != 0
            tmp2 = tl.load(in_ptr1 + (r0_2 + 16*x0), r0_mask & x0mask, eviction_policy='evict_last', other=float("-inf"))
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            rindex = r0_index
            r0_2 = r0_index
            tmp9 = tl.load(in_out_ptr0 + (r0_2 + 16*x0 + 256*x1), r0_mask & x0mask & x1mask, eviction_policy='evict_last', other=0.0)
            tmp10 = tl.load(in_ptr0 + (r0_2 + 16 * x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0) != 0
            tmp11 = tl.load(in_ptr1 + (r0_2 + 16*x0), r0_mask & x0mask, eviction_policy='evict_last', other=float("-inf"))
        ...
        tmp20 = tl.load(in_ptr2 + (x0 + 16 * x1), x0mask & x1mask, eviction_policy='evict_last') != 0
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            r0_index = r0_offset + r0_base
            r0_mask = r0_index < r0_numel
            roffset = r0_offset
            rindex = r0_index
            r0_2 = r0_index
            tmp22 = tl.load(in_out_ptr0 + (r0_2 + 16*x0 + 256*x1), r0_mask & x0mask & x1mask, eviction_policy='evict_first', other=0.0)
            tmp23 = tl.load(in_ptr0 + (r0_2 + 16 * x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0) != 0
            tmp24 = tl.load(in_ptr1 + (r0_2 + 16*x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0)
            tmp21 = tmp20 == 0
            ...
            tl.store(in_out_ptr0 + (r0_2 + 16*x0 + 256*x1), tmp32, r0_mask & x0mask & x1mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused_mul_0.run(arg3_1, buf0, 4096, stream=raw_stream0)
            del arg3_1
            arg2_1 = copy_if_misaligned(arg2_1)
            buf1 = empty_strided_npu((4, 2, 32, 16), (1024, 512, 16, 1), torch.float32)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.transpose, aten.mul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_mul_transpose_1.run(arg2_1, buf1, 256, 16, stream=raw_stream0)
            del arg2_1
            ...
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.full, aten.where, aten.add, aten._safe_softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__safe_softmax_add_full_matmul_where_2.run(buf2, arg1_1, arg0_1, buf3, 2048, stream=raw_stream0)
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.full, aten.where, aten.add, aten._safe_softmax]
            ...
            # Topologically Sorted Source Nodes: [], Original ATen: [aten.matmul, aten.full, aten.where, aten.add, aten._safe_softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__safe_softmax_add_full_matmul_where_3.run(buf7, arg1_1, arg0_1, buf4, 128, 16, stream=raw_stream0)
            del arg0_1
            del arg1_1
            del buf4
            ...
            extern_kernels.bmm(reinterpret_tensor(buf7, (8, 16, 16), (256, 16, 1), 0), reinterpret_tensor(arg4_1, (8, 16, 32), (512, 32, 1), 0), out=buf8)
            del arg4_1
            del buf7
...
def get_args():
    ...
    arg1_1 = rand_strided((16, 16), (16, 1), device='npu:0', dtype=torch.bool)
    ...
    arg4_1 = rand_strided((4, 2, 16, 32), (1024, 512, 32, 1), device='npu:0', dtype=torch.float32)
