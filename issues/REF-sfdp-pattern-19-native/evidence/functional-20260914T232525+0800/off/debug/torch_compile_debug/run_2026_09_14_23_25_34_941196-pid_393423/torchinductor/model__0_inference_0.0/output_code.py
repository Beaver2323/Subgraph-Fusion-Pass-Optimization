...
# kernel path: /home/z50063656/tmp/attention-functional-20260914-unskip/functional-20260914T232525+0800/pattern-19/off/inductor-cache/tmpqnpj512q/57/c57j3mvdtaumkwlytniohiegs52oqyfplvncdbkp5pwmlsajxmee.py
# Topologically Sorted Source Nodes: [attn_weights, inv_scale, attn_weights_1, causal_mask_value, attn_weights_2, attn_weights_3, softmax, attn_weights_4], Original ATen: [aten.matmul, aten.full, aten.div, aten.where, aten.add, aten._softmax, aten._to_copy]
...
#   %div : Tensor "f16[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%view_2, %full_default), kwargs = {})
...
#   %where : Tensor "f16[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%arg2_1, %div, %full_default_1), kwargs = {})
...
#   %convert_element_type_2 : Tensor "f16[2, 4, 8, 8][256, 64, 8, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%div_1, torch.float16), kwargs = {})
#   return %amax,%sum_1,%expand_2
triton_unk_fused__softmax__to_copy_add_div_full_matmul_where_0 = async_compile.triton('triton_unk_fused__softmax__to_copy_add_div_full_matmul_where_0', '''
import triton
import triton.language as tl
...
def triton_unk_fused__softmax__to_copy_add_div_full_matmul_where_0(in_out_ptr0, in_ptr0, in_ptr1, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for i in range(group_size):
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp1 = tl.load(in_out_ptr0 + (r0_2 + 8*x0 + 64*x1), r0_mask & x0mask & x1mask, eviction_policy='evict_last', other=0.0).to(tl.float32)
            tmp7 = tl.load(in_ptr1 + (r0_2 + 8*x0), r0_mask & x0mask, eviction_policy='evict_last', other=float("-inf"))
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            rindex = r0_index
            r0_2 = r0_index
            tmp12 = tl.load(in_ptr0 + (r0_2 + 8 * x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0) != 0
            tmp13 = tl.load(in_out_ptr0 + (r0_2 + 8*x0 + 64*x1), r0_mask & x0mask & x1mask, eviction_policy='evict_last', other=0.0).to(tl.float32)
            tmp19 = tl.load(in_ptr1 + (r0_2 + 8*x0), r0_mask & x0mask, eviction_policy='evict_last', other=float("-inf"))
            tmp14 = tl.full([1, 1], 1.5000150001500014, tl.float32)
            tmp15 = tmp13 * tmp14
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            rindex = r0_index
            r0_2 = r0_index
            tmp26 = tl.load(in_ptr0 + (r0_2 + 8 * x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0) != 0
            tmp27 = tl.load(in_out_ptr0 + (r0_2 + 8*x0 + 64*x1), r0_mask & x0mask & x1mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
            tmp33 = tl.load(in_ptr1 + (r0_2 + 8*x0), r0_mask & x0mask, eviction_policy='evict_last', other=0.0)
            tmp28 = tl.full([1, 1], 1.5000150001500014, tl.float32)
            tmp29 = tmp27 * tmp28
            tmp30 = tl.full([1, 1], -65504.0, tl.float32)
            ...
            tl.store(in_out_ptr0 + (r0_2 + 8*x0 + 64*x1), tmp38, r0_mask & x0mask & x1mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            triton_unk_fused__softmax__to_copy_add_div_full_matmul_where_0.run(buf3, arg2_1, arg3_1, 64, 8, stream=raw_stream0)
            del arg2_1
            del arg3_1
            arg4_1 = copy_if_misaligned(arg4_1)
            buf4 = empty_strided_npu((8, 8, 16), (128, 16, 1), torch.float16)
            # Topologically Sorted Source Nodes: [attn_weights, inv_scale, attn_weights_1, causal_mask_value, attn_weights_2, attn_weights_3, softmax, attn_weights_4, matmul_1], Original ATen: [aten.matmul, aten.full, aten.div, aten.where, aten.add, aten._softmax, aten._to_copy]
...
def get_args():
    ...
    arg4_1 = rand_strided((2, 4, 8, 16), (512, 128, 16, 1), device='npu:0', dtype=torch.float16)
