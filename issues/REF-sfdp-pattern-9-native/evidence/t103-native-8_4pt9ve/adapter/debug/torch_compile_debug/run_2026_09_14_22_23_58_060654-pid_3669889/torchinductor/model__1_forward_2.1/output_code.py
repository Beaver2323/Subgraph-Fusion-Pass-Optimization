...
# kernel path: /home/z50063656/tmp/t103-native-8_4pt9ve/adapter/inductor-cache/tmpwh8gc36j/kp/ckphcqjq76na2rlqu3cgahjh4fuv6esuukrsjofetfd5j5usaica.py
...
# kernel path: /home/z50063656/tmp/t103-native-8_4pt9ve/adapter/inductor-cache/tmpwh8gc36j/ue/cuegvse4q6qaiknsrinclsedyhzkb3f7dj6bqqlqcs6hazcpgm7l.py
...
# kernel path: /home/z50063656/tmp/t103-native-8_4pt9ve/adapter/inductor-cache/tmpwh8gc36j/xs/cxsuty7mnplkikbn4xelg3nvzp5xdslwrbrojrez7orx3j7qsyec.py
# Topologically Sorted Source Nodes: [matmul, to, softmax], Original ATen: [aten.matmul, aten._to_copy, aten._softmax]
# Source node to ATen node mapping:
#   matmul => view_2
...
@triton.jit
def triton_unk_fused__softmax__to_copy_matmul_2(in_ptr0, out_ptr2, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 64
    r0_numel = 8
    ...
    for i in range(group_size):
        ...
        for r0_offset in tl.range(0, r0_numel, R0_BLOCK):
            ...
            tmp16 = (tmp15 / tmp10)
            tl.store(out_ptr2 + (r0_1 + 8*x0), tmp16, r0_mask & x0mask)
''', device_str='npu')


# kernel path: /home/z50063656/tmp/t103-native-8_4pt9ve/adapter/inductor-cache/tmpwh8gc36j/ra/cra3uidtxwifosnja52rkmb4vmxejjk6qv3vnmjilumwnaynlxtk.py
...
#   return %expand_2
triton_unk_fused__to_copy_3 = async_compile.triton('triton_unk_fused__to_copy_3', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
@npu_triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__to_copy_3', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'npu_num_x_nodes': 1, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused__to_copy_3(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    group_id = tl.program_id(0)
    xnumel = 512
...
# kernel path: /home/z50063656/tmp/t103-native-8_4pt9ve/adapter/inductor-cache/tmpwh8gc36j/sm/csmbaye3icl2dqxnpsiddtf4yxqpr2mj3d5y5duch34e2d5vkp22.py
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            del buf1
            del buf2
            buf6 = empty_strided_npu((2, 4, 8, 8), (256, 64, 8, 1), torch.float32)
            # Topologically Sorted Source Nodes: [matmul, to, softmax], Original ATen: [aten.matmul, aten._to_copy, aten._softmax]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__softmax__to_copy_matmul_2.run(buf3, buf6, 64, 8, stream=raw_stream0)
            del buf3
            # Topologically Sorted Source Nodes: [dropout], Original ATen: [aten.native_dropout]
            # [Provenance debug handles] torch.ops.npu._npu_dropout.default:2
            ...
            del buf7
            buf10 = empty_strided_npu((2, 4, 8, 8), (256, 64, 8, 1), torch.float16)
            # Topologically Sorted Source Nodes: [to_1], Original ATen: [aten._to_copy]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__to_copy_3.run(buf8, buf10, 512, stream=raw_stream0)
            del buf8
            primals_3 = copy_if_misaligned(primals_3)
            buf11 = empty_strided_npu((2, 4, 8, 16), (512, 128, 16, 1), torch.float16)
            # Topologically Sorted Source Nodes: [permute_2, matmul_1], Original ATen: [aten.permute, aten.matmul]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_matmul_permute_4.run(primals_3, buf11, 1024, stream=raw_stream0)
            buf12 = empty_strided_npu((8, 8, 16), (128, 16, 1), torch.float16)
            # Topologically Sorted Source Nodes: [permute_2, matmul_1], Original ATen: [aten.permute, aten.matmul]
            # [Provenance debug handles] extern_kernels.bmm:3
            extern_kernels.bmm(reinterpret_tensor(buf10, (8, 8, 8), (64, 8, 1), 0), reinterpret_tensor(buf11, (8, 8, 16), (128, 16, 1), 0), out=buf12)
            del buf11
        return (reinterpret_tensor(buf12, (2, 4, 8, 16), (512, 128, 16, 1), 0), reinterpret_tensor(primals_3, (2, 4, 8, 16), (512, 16, 64, 1), 0), reinterpret_tensor(primals_1, (2, 4, 16, 8), (512, 16, 1, 64), 0), buf0, buf6, buf9, buf10, )
