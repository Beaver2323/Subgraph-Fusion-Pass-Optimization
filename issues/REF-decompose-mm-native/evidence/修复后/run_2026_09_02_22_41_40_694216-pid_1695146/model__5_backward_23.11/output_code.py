...
# kernel path: /home/z50063656/tmp/t077-cache-mm-fixed-suite0/tmp15z4l50i/q6/cq6dqmuiv4yrfq5c6ebkxli563kndz5blrpwllcq5ocwe7fufauw.py
...
# Graph fragment:
#   %mm_1 : Tensor "bf16[2, 2][2, 1]npu:0" = PlaceHolder[target=mm_1]
#   %convert_element_type_8 : Tensor "f32[2, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mm_1, torch.float32), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t077-cache-mm-fixed-suite0/tmp15z4l50i/pc/cpcrrk6yvb4sl47eqnmepggjohmtoog5xlkoj6jgdkev3jis442l.py
...
# Graph fragment:
#   %mm_2 : Tensor "bf16[2048, 2][2, 1]npu:0" = PlaceHolder[target=mm_2]
#   %convert_element_type_9 : Tensor "f32[2048, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mm_2, torch.float32), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((2, 2), (2, 1), torch.bfloat16)
            ...
            del permute
            buf1 = empty_strided_npu((2048, 2), (2, 1), torch.bfloat16)
            ...
            del permute_1
            del tangents_1
            buf2 = empty_strided_npu((2, 2), (2, 1), torch.float32)
            # Topologically Sorted Source Nodes: [convert_element_type_8], Original ATen: [aten._to_copy]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__to_copy_0.run(buf0, buf2, 4, stream=raw_stream0)
            del buf0
            buf3 = empty_strided_npu((2048, 2), (2, 1), torch.float32)
            # Topologically Sorted Source Nodes: [convert_element_type_9], Original ATen: [aten._to_copy]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__to_copy_1.run(buf1, buf3, 4096, stream=raw_stream0)
            del buf1
        return (buf3, buf2, )
...
def get_args():
    ...
    permute = rand_strided((2, 2048), (1, 2), device='npu:0', dtype=torch.bfloat16)
    permute_1 = rand_strided((2, 2), (1, 2), device='npu:0', dtype=torch.bfloat16)
    tangents_1 = rand_strided((2048, 2), (2, 1), device='npu:0', dtype=torch.bfloat16)
