...
# kernel path: /home/z50063656/tmp/t077-cache-mm-fixed-suite0/tmp8777cdyb/dj/cdj5rimoz7odfsionyojhp7fnfylewgkxkqe42vekwt3molntyry.py
...
# Graph fragment:
#   %mm_1 : Tensor "bf16[32, 2][2, 1]npu:0" = PlaceHolder[target=mm_1]
#   %convert_element_type_8 : Tensor "f32[32, 2][2, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mm_1, torch.float32), kwargs = {})
...
# kernel path: /home/z50063656/tmp/t077-cache-mm-fixed-suite0/tmp8777cdyb/at/catgzxb7dmaerjguzvzhievzcw6orqfx55qaypfnyv55oeyqkle5.py
...
# Graph fragment:
#   %mm_2 : Tensor "bf16[20480, 32][32, 1]npu:0" = PlaceHolder[target=mm_2]
#   %convert_element_type_9 : Tensor "f32[20480, 32][32, 1]npu:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mm_2, torch.float32), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            buf0 = empty_strided_npu((32, 2), (2, 1), torch.bfloat16)
            ...
            del permute
            buf1 = empty_strided_npu((20480, 32), (32, 1), torch.bfloat16)
            ...
            del permute_1
            del tangents_1
            buf2 = empty_strided_npu((32, 2), (2, 1), torch.float32)
            # Topologically Sorted Source Nodes: [convert_element_type_8], Original ATen: [aten._to_copy]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__to_copy_0.run(buf0, buf2, 64, stream=raw_stream0)
            del buf0
            buf3 = empty_strided_npu((20480, 32), (32, 1), torch.float32)
            # Topologically Sorted Source Nodes: [convert_element_type_9], Original ATen: [aten._to_copy]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused__to_copy_1.run(buf1, buf3, 655360, stream=raw_stream0)
            del buf1
        return (buf3, buf2, )
...
def get_args():
    ...
    permute = rand_strided((32, 20480), (1, 32), device='npu:0', dtype=torch.bfloat16)
    permute_1 = rand_strided((2, 32), (1, 2), device='npu:0', dtype=torch.bfloat16)
