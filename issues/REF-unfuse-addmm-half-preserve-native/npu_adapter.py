#!/usr/bin/env python3
"""复用上游 half dtype addmm 保留测试；只适配设备与入口。"""
...
METHODS = [
    "test_unfuse_bias_addmm_half_dtypes_bfloat16",
    "test_unfuse_bias_addmm_half_dtypes_float16",
]
...
def main() -> int:
    ...
    spec = importlib.util.spec_from_file_location(
        "t078_unfuse_addmm_half_preserve_upstream", SOURCE
    )
    ...
    result = unittest.TextTestRunner(verbosity=2).run(
        unittest.TestSuite([NpuCase(method) for method in METHODS])
    )
    summary = {
        "tracking_mode": "adapter",
        "source_test": (
            f"{SOURCE}::TestPatternMatcher.test_unfuse_bias_addmm_half_dtypes"
        ),
        "source_sha256": hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        "adapter_deviation": [
            "GPU_TYPE=npu",
            "显式运行原参数化 bf16/fp16 方法，绕过 HAS_GPU 启动门",
            "保留 keep_addmm_fused_for_half_dtypes=true 和原 FileCheck",
        ],
        ...
        "compatibility_verdict": (
            "community-guard-pass"
            if result.wasSuccessful() and result.testsRun == 2
            else "community-guard-fail"
        ),
    }
