...
def source_hashes() -> dict[str, str]:
    ...
    for name, module in list(sys.modules.items()):
        ...
        if not filename or not name.startswith(("torch._inductor", "torch_npu._inductor", "triton")):
            continue
        path = Path(filename)
        if path.is_file():
...
def main() -> None:
    ...
    if args.unit == "stack-normalization":
        ...
        if args.mode == "off":
            ...
            target_control.update(disable_entries(
                split_cat.PRE_GRAD_PATTERNS["normalization_pass"],
                handler_names=TARGETS[args.unit][2],
            ))
        ...
        freezing_patterns.lazy_init(torch.device(device))
        state["registered_handlers"] = install_observer(
            freezing_patterns.binary_folding_pass, TARGETS[args.unit][2], state,
        )
        if state["registered_handlers"] < 1:
            raise RuntimeError("未注册 folded_op handler")
    ...
    record = {
        ...
        "triton_version": getattr(sys.modules.get('triton'), '__version__', None),
        "task_id": task,
        "acceptance_unit_id": acceptance,
        "unit": args.unit,
        "mode": args.mode,
