...
_frozen_param1 = None  # device(type='npu', index=0) torch.float32 (4, 1) (1, 1) fffc4d5f1090
_frozen_param2 = None  # device(type='npu', index=0) torch.float32 (3, 32) (1, 3) fffc64424f00


# kernel path: /home/z50063656/tmp/t100-native-zbaadbq5/adapter/inductor-cache/tmp45ep9j39/2l/c2lwdzo7ctx5icaouyv4unsaumbh6pq5tvontjp5367apvdhph6i.py
# Topologically Sorted Source Nodes: [sub], Original ATen: [aten.sub]
# Source node to ATen node mapping:
#   sub => sub
# Graph fragment:
#   %mm : Tensor "f32[4, 32][32, 1]npu:0" = PlaceHolder[target=mm]
#   %_frozen_param1 : Tensor "f32[4, 1][1, 1]npu:0" = PlaceHolder[target=_frozen_param1]
#   %sub : Tensor "f32[4, 32][32, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%mm, %arg1_1), kwargs = {})
#   return %sub
triton_unk_fused_sub_0 = async_compile.triton('triton_unk_fused_sub_0', '''
import triton
import triton.language as tl
...
@npu_triton_heuristics.pointwise(
    size_hints={'x': 128}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'xnumel': 'i32'}, 'device': DeviceProperties(type='npu', index=0, multi_processor_count=48, cc='Ascend910B2', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, max_threads_per_block=1024, warp_size=None), 'constants': {}, 'mix_mode': 'aiv', 'configs': [AttrsDescriptor.from_dict({'arg_properties': {'tt.divisibility': (0, 1, 2), 'tt.equal_to': ()}, 'cls': 'AttrsDescriptor'})], 'block_hints': {'XBLOCK_HINT': [32, 4, {'divisors': [1, 32]}], 'R0_BLOCK_HINT': [{'divisors': []}]}, 'axis_hints': [{'name': 'x0', 'length': 32, 'divisor': 1, 'seed': 1}, {'name': 'x1', 'length': 4, 'divisor': 32, 'seed': 2}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused_sub_0', 'mutated_arg_names': ['in_out_ptr0'], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 2, 'num_reduction': 0, 'npu_num_x_nodes': 2, 'npu_rsplit_partial': False, 'backend_hash': '573767764059784D1B071A77E46541176356CEAB012EDE3A29BDD4D6B9E2A472', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': False, 'autotune_remote_cache': None, 'force_disable_caches': True, 'dynamic_scale_rblock': True, 'incremental_autotune': False, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'batch_invariant': False, 'force_filter_reduction_configs': False, 'mix_order_reduction_allow_multi_stages': True, 'dynamic_disable_pipelining': True, 'are_deterministic_algorithms_enabled': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_unk_fused_sub_0(in_out_ptr0, in_ptr0, xnumel, XBLOCK : tl.constexpr):
    total_thread = 48
    ...
    for i in range(group_size):
        ...
        xmask = x0mask & x1mask
        tmp0 = tl.load(in_out_ptr0 + (x0 + 32*x1), x0mask & x1mask)
        tmp1 = tl.load(in_ptr0 + (x1), x1mask, eviction_policy='evict_last')
        tmp2 = tmp0 - tmp1
        tl.store(in_out_ptr0 + (x0 + 32*x1), tmp2, x0mask & x1mask)
''', device_str='npu')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten.matmul]
            # [Provenance debug handles] extern_kernels.mm:1
            extern_kernels.mm(arg2_1, _frozen_param2, out=buf0)
            del arg2_1
            buf1 = buf0; del buf0  # reuse
            # Topologically Sorted Source Nodes: [sub], Original ATen: [aten.sub]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_sub_0.run(buf1, _frozen_param1, 128, stream=raw_stream0)
        return (buf1, )
...
def get_args():
    ...
    _frozen_param1 = rand_strided((4, 1), (1, 1), device='npu:0', dtype=torch.float32)
    global _frozen_param2
    _frozen_param2 = rand_strided((3, 32), (1, 3), device='npu:0', dtype=torch.float32)
    arg2_1 = rand_strided((4, 3), (3, 1), device='npu:0', dtype=torch.float32)
    return [arg2_1]
