#!/usr/bin/env python3
"""T-078 baddbmm bias unfuse 核心社区用例的原生入口预检与适配调度。"""
...
METHOD = "TestPatternMatcher.test_unfuse_broadcast_bias_baddbmm"
...
mode.add_argument(
    "--codegen-adapter",
    action="store_true",
    help="在已记录 CUDA codegen needle blocker 后规范化 NPU direct-ATen 符号",
)
args = parser.parse_args()
if Path.cwd().resolve() != WORK:
    raise SystemExit("必须从 /home/z50063656/tmp 启动")
root = WORK / "t078-npu-results" / "REF-unfuse-baddbmm-core-native"
root.mkdir(parents=True, exist_ok=True)
prefix = (
    "codegen-adapter-"
    if args.codegen_adapter
...
if args.adapter or args.codegen_adapter:
    command = [
        sys.executable,
    ...
    if args.codegen_adapter:
        command.append("--normalize-codegen-symbols")
with (run / "stdout.log").open("w") as out, (run / "stderr.log").open("w") as err:
    try:
        result = subprocess.run(
            command, cwd=WORK, env=env, stdout=out, stderr=err, timeout=600
        )
        status = result.returncode
    except subprocess.TimeoutExpired:
        status = 124
summary = {
    "source_test": f"{SOURCE}::{METHOD}",
    "return_code": status,
    "tracking_mode": (
