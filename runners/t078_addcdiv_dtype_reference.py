...
value=2、eager/compiled 对照和 FMA/div_rn codegen 判据，只改变 dtype。
"""
...
def main() -> int:
    args = parse_args()
    if args.source_test != EXPECTED_SOURCE_TEST:
    ...
    class AddcdivDtypeReference(unittest.TestCase):
        def test_dtype_extension(self) -> None:
            ...
            print(
                f"dtype={args.dtype} bitwise_equal={torch.equal(actual, expected)} "
