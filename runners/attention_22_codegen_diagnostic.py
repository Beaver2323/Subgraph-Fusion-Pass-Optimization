...
def broadcast_slice_candidate(source):
    """只对该生成图四个共享mask加载恢复singleton维；不是产品补丁。"""
    expression = re.compile(
        r"(?P<load>_es_full(?P<n>\d+) = tl\.load\(in_ptr1[^\n]+\n)"
        r"(?P<slice>        tmp\d+ = extract_slice\(_es_full(?P=n), \[0, 0, 0\], \[)"
        r"real_block_x2(?P<tail>, real_block_x1, 1\], \[1, 1, 1\]\))"
    )
    updated, count = expression.subn(r"\g<load>\g<slice>1\g<tail>", source)
    if count != 4:
        raise ValueError(f'固定生成图应有4处共享mask加载，实际{count}；拒绝猜测替换')
    return updated
...
def main():
    ...
    p.add_argument('--broadcast-slice-candidate', action='store_true')
    args = p.parse_args()
    ...
    (output/'original_generated_source.py').write_bytes(SOURCE.read_bytes())
    if args.broadcast_slice_candidate:
        source = broadcast_slice_candidate(source)
    (output/'generated_source.py').write_text(source)
    import torch
    import torch_npu
    ...
    record = dict(generated_at=datetime.now().astimezone().isoformat(),
                  ...
                  candidate='mask-broadcast-extract-shape-only' if args.broadcast_slice_candidate else None,
                  executed_generated_source_sha256=digest(output/'generated_source.py'),
