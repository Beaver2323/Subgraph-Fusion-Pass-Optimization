...
partitioned-scatter尚无合法NPU ON产品决策，不提供执行入口。
EOF
}
...
case "${unit}" in
  ...
  overlap-device-put)
    ...
    ;;
  partitioned-scatter)
    echo "错误：partitioned-scatter处于capability-pending，拒绝制造ON路径。" >&2
    exit 2
    ;;
...
[[ -f "${gate}" ]] || { echo "错误：缺少人工签署gate ${gate}" >&2; exit 2; }

exec python "${repo_root}/scripts/run_t085_performance.py" \
  --unit "${unit}" \
  --device npu \
  --gate "${gate}"
