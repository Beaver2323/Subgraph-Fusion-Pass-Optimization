...
npu_id="${1:-0}"
if [[ ! "${npu_id}" =~ ^[0-9]+$ ]]; then
    echo "用法：bash scripts/verify_t080_scatter_gate.sh [NPU_ID]" >&2
    exit 2
fi
...
source /home/z50063656/Pass/activate_pass.sh \
    >/tmp/t080-scatter-gate-activate.log 2>&1
activate_status=$?
set -e
if ((activate_status != 0)); then
    echo "错误：Pass 环境激活失败，详见 /tmp/t080-scatter-gate-activate.log" >&2
...
run_id="scatter-gate-verify-$(date +%Y%m%dT%H%M%S%z)"
output_root="/home/z50063656/tmp/t080-scatter-gate-results/${run_id}"
mkdir -p "${output_root}"

for mode in default-disabled gate-disabled; do
    mode_dir="${output_root}/${mode}"
    mkdir -p "${mode_dir}"
    python "${repo_root}/runners/verify_t080_scatter_gate.py" \
        --mode "${mode}" \
