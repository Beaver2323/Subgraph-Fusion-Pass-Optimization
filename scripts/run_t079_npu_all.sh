...
npu_id="${1:-0}"
if [[ ! "${npu_id}" =~ ^[0-9]+$ ]]; then
    echo "用法：bash scripts/run_t079_npu_all.sh [NPU_ID]" >&2
    exit 2
fi

# shellcheck disable=SC1091
set +e
source /home/z50063656/Pass/activate_pass.sh >/tmp/t079-npu-activate.log
activate_status=$?
set -e
if ((activate_status != 0)); then
    echo "错误：Pass 环境激活失败，详见 /tmp/t079-npu-activate.log" >&2
...
for case_id in "${cases[@]}"; do
    ...
    python "${repo_root}/runners/run_t079_case.py" --case-id "${case_id}"
    echo "END ${case_id} direct status=NO_TESTS"
    echo "START ${case_id} adapter"
    python "${repo_root}/runners/run_t079_case.py" --case-id "${case_id}" --adapter
