...
def _register_npu_addcdiv_lowering():
    """Lower NPU addcdiv with backend-accurate floating-point semantics.

    This must run after ``_register_npu_inductor_fallbacks`` because addcdiv is
    intentionally absent from the backend's broad GENERATE_LIST.  Registering
    only the exact default overload keeps the capability exception narrow.

    FP32/BF16 use round-to-nearest division followed by fused multiply-add.
    NPU eager FP16 instead exposes a low-precision boundary after division and
    multiplication.  Preserve those boundaries inside one Triton kernel;
    applying the FP32-style FMA to FP16 changes observable results.
    """
        ...
        def inner_fn(idx):
            ...
            if dtype == torch.float16:
                ...
                if isinstance(value, sympy.Basic):
                    ...
                    # A wrapped Python scalar is converted to FP16 by eager
                    # tensor arithmetic.  Quantize the compile-time literal
                    # before Triton promotes it to its float32 compute type.
            ...
            if isinstance(value, sympy.Basic):
                ...
                # NPU eager converts the scalar to the output dtype before
                # promoting BF16 arithmetic to FP32 for division and FMA.
                scalar_value = (
                    torch.tensor(value, dtype=torch.bfloat16).item()
                    if dtype == torch.bfloat16 else value
                )
                value_expr = ops.constant(scalar_value, dtype)
...
def _register_npu_fp16_div_lowering():
    """Preserve the eager FP16 quotient boundary in fused NPU pointwise code.

    Triton promotes FP16 loads to its FP32 compute type.  Without an explicit
    round trip, ``div -> consumer`` fusion keeps the quotient in FP32 until the
    final store, while the NPU eager ``aten.div`` result is materialized in
    FP16 before the consumer runs.  Keep every non-FP16 path on the upstream
    lowering and make only the NPU FP16 tensor overload expose that boundary.
    """
    original = lowerings.get(aten.div.Tensor)
    if original is None or getattr(
        original, "_npu_fp16_div_rounding_installed", False
    ):
        return
    ...
    npu_fp16_div._npu_fp16_div_rounding_installed = True
    npu_fp16_div._npu_fp16_div_rounding_original = original
...
def _register_npu_inductor_fallbacks():
    ...
    _register_npu_fp16_div_lowering()
