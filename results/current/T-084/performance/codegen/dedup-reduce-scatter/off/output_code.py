...
# kernel path: /home/z50063656/tmp/t084-performance-results/benchmark-dedup-reduce-scatter-20260910T055903-704gzsgo/off1/rank-0/inductor-cache/tmp3383ckpb/47/c47dqxxpsbgbphj4xujdaupqarhfset4qhf7pdavh3wrdbazhhsd.py
# Topologically Sorted Source Nodes: [add], Original ATen: [aten.add]
# Source node to ATen node mapping:
#   add => add
# Graph fragment:
#   %wait_tensor : Tensor "f32[2, 128][128, 1]npu:0" = PlaceHolder[target=wait_tensor]
#   %wait_tensor_1 : Tensor "f32[2, 128][128, 1]npu:0" = PlaceHolder[target=wait_tensor_1]
#   %add : Tensor "f32[2, 128][128, 1]npu:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%wait_tensor, %wait_tensor_1), kwargs = {})
#   return %add
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.npu.utils.device(0):
            ...
            # Topologically Sorted Source Nodes: [first_out], Original ATen: [_c10d_functional.reduce_scatter_tensor]
            # [Provenance debug handles] torch.ops._c10d_functional.reduce_scatter_tensor.default:1
            buf0 = torch.ops._c10d_functional.reduce_scatter_tensor.default(arg0_1, 'avg', 2, '0')
            assert_alignment(buf0, 16, 'torch.ops._c10d_functional.reduce_scatter_tensor.default')
            del arg0_1
            arg1_1 = copy_if_misaligned(arg1_1)
            # Topologically Sorted Source Nodes: [second_out], Original ATen: [_c10d_functional.reduce_scatter_tensor]
            # [Provenance debug handles] torch.ops._c10d_functional.reduce_scatter_tensor.default:2
            buf1 = torch.ops._c10d_functional.reduce_scatter_tensor.default(arg1_1, 'avg', 2, '0')
            ...
            # [Provenance debug handles] torch.ops._c10d_functional.wait_tensor.default:3
            buf2 = torch.ops._c10d_functional.wait_tensor.default(buf0)
            assert_alignment(buf2, 16, 'torch.ops._c10d_functional.wait_tensor.default')
            del buf0
            # Topologically Sorted Source Nodes: [wait_tensor_1], Original ATen: [_c10d_functional.wait_tensor]
            # [Provenance debug handles] torch.ops._c10d_functional.wait_tensor.default:4
            buf3 = torch.ops._c10d_functional.wait_tensor.default(buf1)
            assert_alignment(buf3, 16, 'torch.ops._c10d_functional.wait_tensor.default')
            del buf1
            buf4 = buf2; del buf2  # reuse
            # Topologically Sorted Source Nodes: [add], Original ATen: [aten.add]
            raw_stream0 = get_raw_stream(0)
            triton_unk_fused_add_0.run(buf4, buf3, 256, stream=raw_stream0)
            del buf3
        return (buf4, )
...
def get_args():
    ...
    arg0_1 = rand_strided((4, 128), (128, 1), device='npu:0', dtype=torch.float32)
    arg1_1 = rand_strided((4, 128), (128, 1), device='npu:0', dtype=torch.float32)
