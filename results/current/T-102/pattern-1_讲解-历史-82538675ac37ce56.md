...
> 更新时间：2026-09-15 19:58 CST（UTC+08:00）。原社区合同、安装修复、精确OFF/ON与性能处置完成；不是产品PR已合入。
...
按本编号实际改写计数，不以总 fuse_attention counter 替代精确归因。 当前NPU生成代码不等同CUDA内核；FX改写不自动等于融合FA或性能收益。
