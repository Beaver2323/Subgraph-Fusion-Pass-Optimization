# T-087：训练局部性重排 功能与性能报告

> 更新时间：2026-09-11 15:25 CST（UTC+08:00）

## 1. Pattern 意图与代码

使同一反向分支尽早计算和释放；只调整拓扑合法的顺序，不改变算子数学定义。

```python
# 文件：torch/_inductor/fx_passes/post_grad.py:193
reorder_for_locality(graph)  # 只在允许的训练/主开关下调用
```

## 2. GPU / NPU 行为对照

GPU：社区训练 OFF/ON 验证参数、梯度与节点移动，主开关关闭时零调用。

NPU：相同训练合同通过；反向 matmul_backward 在 NPU 保留为图内原生 extern，不能声称 CUDA 相同反向 lowering。

NPU 后端为 `triton_experimental`。原生入口先记录跳过/不生成测试类，再做设备 harness 适配；没有放宽社区数值断言或覆盖冻结源码。

原例步骤、最小适配和失败记录见 [适配报告](../../../issues/REF-reorder-locality-training-native/适配报告.md)。
...
结论：`PERF_MIXED`；保留现有默认配置，不凭一次局部图收益全局开启。
...
原始样本、显存、编译时间及逐臂 FX/IR/output_code 索引见 [性能汇总](performance_summary.json)；[功能与门禁原件](functional/reorder-locality.json)。
